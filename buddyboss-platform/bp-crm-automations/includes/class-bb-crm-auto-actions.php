...
class BB_CRM_Auto_Actions {
	...
	private static function action_assign_tag( $config, $user_id ) {
		...
		if ( function_exists( 'bb_crm_add_user_tag' ) ) {
			...
			$exists = $wpdb->get_var( $wpdb->prepare(
				"SELECT id FROM {$wpdb->prefix}bp_user_tags WHERE user_id = %d AND tag_id = %d",
				$user_id, $tag_id
			) );
			if ( ! $exists ) {
				$wpdb->insert( $wpdb->prefix . 'bp_user_tags', array( 'user_id' => $user_id, 'tag_id' => $tag_id, 'source' => 'automation' ) );
			}
			$result = true;
		}
	...
	private static function action_remove_tag( $config, $user_id ) {
		...
		if ( function_exists( 'bb_crm_remove_user_tag' ) ) {
			...
			global $wpdb;
			$result = $wpdb->delete( $wpdb->prefix . 'bp_user_tags', array( 'user_id' => $user_id, 'tag_id' => $tag_id ) );
		}
	...
	private static function action_add_to_list( $config, $user_id ) {
		...
		global $wpdb;
		$exists = $wpdb->get_var( $wpdb->prepare(
			"SELECT id FROM {$wpdb->prefix}bp_crm_list_assignments WHERE list_id = %d AND user_id = %d",
			$list_id, $user_id
		) );

		if ( ! $exists ) {
			$wpdb->insert( $wpdb->prefix . 'bp_crm_list_assignments', array( 'list_id' => $list_id, 'user_id' => $user_id ) );
		}
	...
	private static function action_remove_from_list( $config, $user_id ) {
		...
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . 'bp_crm_list_assignments', array( 'list_id' => $list_id, 'user_id' => $user_id ) );
	...
	private static function action_send_campaign_email( $config, $user_id, $trigger_data ) {
		...
		global $wpdb;
		$camp_table = $wpdb->prefix . 'bp_crm_campaigns';
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		$wpdb->insert( $wpdb->prefix . 'bp_crm_email_opens', array(
			'user_id'     => $user_id,
	...
	public static function get_available_actions() {
		$actions = array(
			'wait'              => __( '⏱ Wait / Delay', 'buddyboss-crm-automations' ),
			'loop_repeat'       => __( '🔁 Wait & Repeat from Start', 'buddyboss-crm-automations' ),
			'cancel_sequence'   => __( '🛑 Cancel Pending Sequence', 'buddyboss-crm-automations' ),
			'assign_tag'        => __( 'Assign Tag', 'buddyboss-crm-automations' ),
			'remove_tag'        => __( 'Remove Tag', 'buddyboss-crm-automations' ),
			'add_to_list'       => __( 'Add to List', 'buddyboss-crm-automations' ),
			'remove_from_list'  => __( 'Remove from List', 'buddyboss-crm-automations' ),
			'send_email'        => __( 'Send Custom Email', 'buddyboss-crm-automations' ),
			'call_webhook'      => __( 'Call Webhook', 'buddyboss-crm-automations' ),
			'log_activity'      => __( 'Log Activity Note', 'buddyboss-crm-automations' ),
		);

		if ( defined( 'BB_CRM_CAMP_VERSION' ) ) {
			$actions['send_campaign_email'] = __( 'Send Campaign Email', 'buddyboss-crm-automations' );
		}

		if ( class_exists( 'BB_Camp_Unsubscribe' ) ) {
			$actions['subscribe_email']   = __( 'Subscribe to Email Campaigns', 'buddyboss-crm-automations' );
			$actions['unsubscribe_email'] = __( 'Unsubscribe from Email Campaigns', 'buddyboss-crm-automations' );
		}
