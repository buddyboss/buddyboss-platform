...
class BB_CRM_Auto_Conditions {
	...
	private static function cond_has_opened_email( $config, $user_id ) {
		...
		global $wpdb;
		return (bool) $wpdb->get_var( $wpdb->prepare(
			"SELECT id FROM {$wpdb->prefix}bp_crm_email_opens WHERE user_id = %d AND campaign_id = %d AND opened_at IS NOT NULL LIMIT 1",
			$user_id, $campaign_id
		) );
	}
	...
	public static function get_available_conditions() {
		return apply_filters( 'bb_crm_auto_available_conditions', array(
			'has_tag'           => __( 'Has Tag', 'buddyboss-crm-automations' ),
			'not_has_tag'       => __( 'Does Not Have Tag', 'buddyboss-crm-automations' ),
			'in_list'           => __( 'Is In List', 'buddyboss-crm-automations' ),
			'not_in_list'       => __( 'Is Not In List', 'buddyboss-crm-automations' ),
			'user_role'         => __( 'Has User Role', 'buddyboss-crm-automations' ),
			'in_group'          => __( 'Is In Group', 'buddyboss-crm-automations' ),
			'profile_field'     => __( 'Profile Field Value', 'buddyboss-crm-automations' ),
			'registration_days' => __( 'Days Since Registration', 'buddyboss-crm-automations' ),
			'tag_count'         => __( 'Total Tag Count', 'buddyboss-crm-automations' ),
			'has_opened_email'  => __( 'Opened Email', 'buddyboss-crm-automations' ),
		) );
	}
}
