...
$page_title = $is_new ? __( 'New Automation', 'buddyboss-crm-automations' ) : __( 'Edit Automation', 'buddyboss-crm-automations' );
?>
...
<div class="wrap bb-crm-wrap">
	...
	<?php if ( isset( $_GET['saved'] ) ) : ?>
		<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Automation saved.', 'buddyboss-crm-automations' ); ?></p></div>
	<?php endif; ?>
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="bb-crm-auto-edit-form">
		...
		<div class="bb-crm-auto-edit-layout">
			...
			<div class="bb-crm-auto-main">
				...
				<!-- Name & Description -->
				<div class="postbox">
					<div class="postbox-header"><h2><?php esc_html_e( 'Automation Details', 'buddyboss-crm-automations' ); ?></h2></div>
					<div class="inside">
						<table class="form-table">
							<tr>
								<th><label for="auto-name"><?php esc_html_e( 'Name', 'buddyboss-crm-automations' ); ?></label></th>
								<td><input type="text" id="auto-name" name="name" value="<?php echo esc_attr( $automation->name ?? '' ); ?>" class="regular-text" required></td>
							</tr>
							<tr>
								<th><label for="auto-desc"><?php esc_html_e( 'Description', 'buddyboss-crm-automations' ); ?></label></th>
			...
			?>
			<div class="postbox">
				<div class="postbox-header"><h2><?php esc_html_e( 'Trigger', 'buddyboss-crm-automations' ); ?></h2></div>
				<div class="inside">
					...
					<div id="bb-crm-trigger-state-0"<?php echo $cur_trigger_def ? ' style="display:none"' : ''; ?>>
						<p class="bb-crm-trigger-question">
							<?php esc_html_e( 'What type of automation would you like to create?', 'buddyboss-crm-automations' ); ?>
						</p>
						<div class="bb-crm-trigger-cat-grid">
							...
							<div class="bb-crm-trigger-cat-card" data-category="<?php echo esc_attr( $cat_key ); ?>">
								...
								<span class="bb-crm-cat-count"><?php printf( _n( '%d trigger', '%d triggers', $count, 'buddyboss-crm-automations' ), $count ); ?></span>
							</div>
							<?php endforeach; ?>
						</div>
					</div>
					...
					<div id="bb-crm-trigger-state-1" style="display:none">
						<div class="bb-crm-step2-header">
							<button type="button" id="bb-crm-back-to-cats" class="button button-small">
								&larr; <?php esc_html_e( 'Back', 'buddyboss-crm-automations' ); ?>
							</button>
					...
					<div id="bb-crm-trigger-state-2"<?php echo $cur_trigger_def ? '' : ' style="display:none"'; ?>>
						<div class="bb-crm-trigger-selected">
							...
							<button type="button" id="bb-crm-change-trigger" class="button button-small">
								<?php esc_html_e( 'Change Trigger', 'buddyboss-crm-automations' ); ?>
							</button>
						</div>
					</div>
				...
				<!-- Actions -->
				<div class="postbox">
					<div class="postbox-header"><h2><?php esc_html_e( 'Workflow Steps', 'buddyboss-crm-automations' ); ?></h2></div>
					<div class="inside">
						<p class="description"><?php esc_html_e( 'Steps run in order when the trigger fires. Mix actions, waits, and condition checks freely.', 'buddyboss-crm-automations' ); ?></p>
								...
								<div class="bb-crm-action-row<?php echo $is_cond ? ' bb-crm-condition-step' : ''; ?>" data-index="<?php echo $ai; ?>">
									...
									<?php else : ?>
										<select name="action_type[<?php echo $ai; ?>]" class="bb-crm-action-type">
											<option value=""><?php esc_html_e( '— Select action —', 'buddyboss-crm-automations' ); ?></option>
									...
									<?php endif; ?>
									<button type="button" class="button button-small bb-crm-remove-action"><?php esc_html_e( 'Remove', 'buddyboss-crm-automations' ); ?></button>
								</div>
							<?php endforeach; ?>
						</div>
						<div class="bb-crm-workflow-add-buttons">
							<button type="button" id="bb-crm-add-action" class="button"><?php esc_html_e( '+ Add Action', 'buddyboss-crm-automations' ); ?></button>
							<button type="button" id="bb-crm-add-condition-step" class="button button-condition"><?php esc_html_e( '+ Add Condition', 'buddyboss-crm-automations' ); ?></button>
						</div>
					</div>
				</div>
			...
			<div class="bb-crm-auto-sidebar">
				<div class="postbox">
					<div class="postbox-header"><h2><?php esc_html_e( 'Publish', 'buddyboss-crm-automations' ); ?></h2></div>
					<div class="inside">
						<label for="auto-status"><?php esc_html_e( 'Status:', 'buddyboss-crm-automations' ); ?></label>
						<select name="status" id="auto-status">
							<option value="active" <?php selected( $automation->status ?? 'active', 'active' ); ?>><?php esc_html_e( 'Active', 'buddyboss-crm-automations' ); ?></option>
							<option value="inactive" <?php selected( $automation->status ?? '', 'inactive' ); ?>><?php esc_html_e( 'Inactive', 'buddyboss-crm-automations' ); ?></option>
							<option value="draft" <?php selected( $automation->status ?? '', 'draft' ); ?>><?php esc_html_e( 'Draft', 'buddyboss-crm-automations' ); ?></option>
						</select>
						<br><br>
						<label for="auto-priority"><?php esc_html_e( 'Priority:', 'buddyboss-crm-automations' ); ?></label>
						...
						<p class="description"><?php esc_html_e( 'Lower number = runs first.', 'buddyboss-crm-automations' ); ?></p>
						<br>
						<button type="submit" class="button button-primary button-large" style="width:100%">
							<?php esc_html_e( 'Save Automation', 'buddyboss-crm-automations' ); ?>
						</button>
					</div>
				</div>
				...
				<?php if ( ! $is_new ) : ?>
				<div class="postbox">
					<div class="postbox-header"><h2><?php esc_html_e( 'Stats', 'buddyboss-crm-automations' ); ?></h2></div>
					<div class="inside">
						<p><strong><?php esc_html_e( 'Total runs:', 'buddyboss-crm-automations' ); ?></strong> <?php echo number_format( $automation->run_count ); ?></p>
						<p><strong><?php esc_html_e( 'Last run:', 'buddyboss-crm-automations' ); ?></strong> <?php echo $automation->last_run ? esc_html( human_time_diff( strtotime( $automation->last_run ) ) . ' ago' ) : '—'; ?></p>
						<a href="<?php echo esc_url( admin_url( 'admin.php?page=buddyboss-crm-automation-log&automation_id=' . $automation->id ) ); ?>" class="button button-small">
							<?php esc_html_e( 'View Log', 'buddyboss-crm-automations' ); ?>
						</a>
					</div>
				</div>
				<?php endif; ?>
...
function bb_crm_auto_render_action_config( $index, $type, $config ) {
	...
	switch ( $type ) {
		...
		case 'subscribe_email':
			$output = '<span style="color:#16a34a;font-size:13px">✓ ' . esc_html__( 'Re-subscribes the user to email campaigns (removes from unsubscribe list).', 'buddyboss-crm-automations' ) . '</span>';
			break;
		case 'unsubscribe_email':
			$output = '<span style="color:#dc2626;font-size:13px">✗ ' . esc_html__( 'Unsubscribes the user from email campaigns (adds to unsubscribe list).', 'buddyboss-crm-automations' ) . '</span>';
			break;
		case 'wait':
			...
			$output .= '</select>';
			$output .= ' <span style="color:#6b7280;font-size:12px">' . esc_html__( '— then continue to the next action', 'buddyboss-crm-automations' ) . '</span>';
			break;
		case 'loop_repeat':
		...
		case 'check_condition':
			...
			$output .= '<option value="">' . esc_html__( '— Select condition —', 'buddyboss-crm-automations' ) . '</option>';
			...
			$output .= '</select>';
			$output .= '<label style="white-space:nowrap"><input type="checkbox" name="action_config[' . $index . '][negate]" value="1" ' . checked( $negate, true, false ) . '> ' . esc_html__( 'NOT (stop if condition IS true)', 'buddyboss-crm-automations' ) . '</label>';
			$output .= '</div>';
			...
			if ( $negate ) {
				$output .= '⚠ ' . esc_html__( 'Sequence stops if condition IS true (NOT checked).', 'buddyboss-crm-automations' );
			} else {
				$output .= '⚠ ' . esc_html__( 'Sequence stops if condition is false.', 'buddyboss-crm-automations' );
			}
			$output .= '</p>';
			break;
	}
...
function bb_crm_auto_tag_options( $selected = 0 ) {
	global $wpdb;
	$tags = $wpdb->get_results( "SELECT id, name FROM {$wpdb->prefix}bp_tags ORDER BY name ASC" );
	$html = '<option value="">' . esc_html__( '— Select tag —', 'buddyboss-crm-automations' ) . '</option>';
...
function bb_crm_auto_list_options( $selected = 0 ) {
	global $wpdb;
	$lists = $wpdb->get_results( "SELECT id, name FROM {$wpdb->prefix}bp_user_lists ORDER BY name ASC" );
	$html  = '<option value="">' . esc_html__( '— Select list —', 'buddyboss-crm-automations' ) . '</option>';
...
function bb_crm_auto_automation_options( $selected = 0 ) {
	...
	$html = '<option value="0"' . selected( $selected, 0, false ) . '>' . esc_html__( 'All pending sequences', 'buddyboss-crm-automations' ) . '</option>';
...
function bb_crm_auto_campaign_options( $selected = 0 ) {
	...
	$html = '<option value="">' . esc_html__( '— Select campaign —', 'buddyboss-crm-automations' ) . '</option>';
...
function bb_crm_auto_inline_condition_config( $index, $type, $config ) {
	...
	switch ( $type ) {
		...
		case 'user_role':
			...
			$output = '<select name="' . $p . '[role]"><option value="">' . esc_html__( '— Select role —', 'buddyboss-crm-automations' ) . '</option>';
...
function self_or_static_render_condition_config( $index, $type, $config ) {
	...
	switch ( $type ) {
		...
		case 'user_role':
			...
			$output  .= '<option value="">' . esc_html__( '— Select role —', 'buddyboss-crm-automations' ) . '</option>';
			foreach ( $roles as $role_key => $role_label ) {
				$output .= '<option value="' . esc_attr( $role_key ) . '" ' . selected( $config['role'] ?? '', $role_key, false ) . '>' . esc_html( $role_label ) . '</option>';
			}
			$output .= '</select>';
