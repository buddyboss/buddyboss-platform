...
use BuddyBossPlatform\Doctrine\Common\Cache\FileCache;
use BuddyBossPlatform\Doctrine\Tests\DoctrineTestCase;
...
class FileCacheTest extends DoctrineTestCase
{
    ...
    {
        $this->driver = $this->getMockBuilder(FileCache::class)->setMethods(['doFetch', 'doContains', 'doSave'])->disableOriginalConstructor()->getMock();
    }
    ...
    {
        $driver1 = $this->getMockBuilder(FileCache::class)->setMethods(['doFetch', 'doContains', 'doSave'])->setConstructorArgs([__DIR__, '.*'])->getMock();
        $driver2 = $this->getMockBuilder(FileCache::class)->setMethods(['doFetch', 'doContains', 'doSave'])->setConstructorArgs([__DIR__, '.php'])->getMock();
    ...
    {
        $driver = $this->getMockBuilder(FileCache::class)->setMethods(['doFetch', 'doContains', 'doSave'])->setConstructorArgs([__DIR__ . '/../', DIRECTORY_SEPARATOR . basename(__FILE__)])->getMock();
    ...
    public function testNonIntUmaskThrowsInvalidArgumentException() : void
    {
        ...
        $this->getMockBuilder(FileCache::class)->setMethods(['doFetch', 'doContains', 'doSave'])->setConstructorArgs(['', '', 'invalid'])->getMock();
    }
    public function testGetDirectoryReturnsRealpathDirectoryString() : void
    {
        $directory = __DIR__ . '/../';
        $driver = $this->getMockBuilder(FileCache::class)->setMethods(['doFetch', 'doContains', 'doSave'])->setConstructorArgs([$directory])->getMock();
    ...
    public function testGetExtensionReturnsExtensionString() : void
    {
        ...
        $driver = $this->getMockBuilder(FileCache::class)->setMethods(['doFetch', 'doContains', 'doSave'])->setConstructorArgs([$directory, $extension])->getMock();
    ...
    public static function getBasePathForWindowsPathLengthTests(int $pathLength) : string
    {
        ...
        $basePath = realpath(sys_get_temp_dir()) . '/' . uniqid('doctrine-cache', \true);
    ...
    public static function getKeyAndPathFittingLength(int $length, string $basePath) : array
    {
        ...
        $keyLength = $length - ($baseDirLength + $extensionLength + $directoryLength);
        // - 1 because of slash
        $key = str_repeat('a', floor($keyLength / 2));
        $keyHash = hash('sha256', $key);
        $keyPath = $basePath . DIRECTORY_SEPARATOR . substr($keyHash, 0, 2) . DIRECTORY_SEPARATOR . bin2hex($key) . '.doctrine.cache';
        $hashedKeyPath = $basePath . DIRECTORY_SEPARATOR . substr($keyHash, 0, 2) . DIRECTORY_SEPARATOR . '_' . $keyHash . '.doctrine.cache';
    ...
    public function getPathLengthsToTest() : array
    {
        ...
        return [[257, \false], [258, \false], [259, \true], [260, \true]];
    }
    /**
    ...
    public function testWindowsPathLengthLimitationsAreCorrectlyRespected(int $length, bool $pathShouldBeHashed) : void
    {
        ...
        $fileCache = $this->getMockForAbstractClass('BuddyBossPlatform\\Doctrine\\Common\\Cache\\FileCache', [$basePath, '.doctrine.cache']);
        ...
        $getFileName->setAccessible(\true);
        self::assertEquals($length, strlen($keyPath), sprintf('Path expected to be %d characters long is %d characters long', $length, strlen($keyPath)));
        if ($pathShouldBeHashed) {
            $keyPath = $hashedKeyPath;
        }
        if ($pathShouldBeHashed) {
            self::assertSame($hashedKeyPath, $getFileName->invoke($fileCache, $key), 'Keys should be hashed correctly if they are over the limit.');
        } else {
            self::assertSame($keyPath, $getFileName->invoke($fileCache, $key), 'Keys below limit of the allowed length are used directly, unhashed');
        }
    }
}
