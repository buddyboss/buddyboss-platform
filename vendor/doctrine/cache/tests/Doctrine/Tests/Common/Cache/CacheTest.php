...
abstract class CacheTest extends DoctrineTestCase
{
    ...
    public function testFetchMultiple() : void
    {
        ...
        self::assertEquals($saved, $cache->fetchMultiple($keys), 'Testing fetchMultiple with different data types');
        self::assertEquals(array_slice($saved, 0, 1), $cache->fetchMultiple(array_slice($keys, 0, 1)), 'Testing fetchMultiple with a single key');
        $keysWithNonExisting = [];
        ...
        self::assertEquals(array_slice($saved, 0, 2), $cache->fetchMultiple($keysWithNonExisting), 'Testing fetchMultiple with a subset of keys and mixed with non-existing ones');
    }
    ...
    public function provideDataToCache() : array
    {
        ...
        return [
            'array' => [['one', 2, 3.01]],
            'string' => ['value'],
            'string_invalid_utf8' => ["\xc3("],
            'string_null_byte' => ['with' . "\x00" . 'null char'],
            'integer' => [1],
            'float' => [1.5],
    ...
    public function testDeleteAll() : void
    {
        ...
        self::assertFalse($cache->contains('key1'), sprintf('key1 should have disappeared but did not. The namespace is "%s"', $cache->getNamespace()));
    ...
    public function testFetchingANonExistingKeyShouldNeverCauseANoticeOrWarning() : void
    {
        ...
        $cache->fetch('key');
        self::assertSame($errorHandler, set_error_handler(static function () {
    ...
    protected abstract function getCacheDriver() : CacheProvider;
}
