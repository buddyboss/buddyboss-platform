...
use BuddyBossPlatform\Doctrine\Common\Cache\ChainCache;
class ChainCacheTest extends CacheTest
{
    ...
    {
        $cache1 = new ArrayCache();
        $cache2 = $this->getMockBuilder(CacheProvider::class)->setMethods(['doFetchMultiple'])->getMockForAbstractClass();
    ...
    public function testFetchPropagatesToFastestCacheUsingDefaultLifeTimeForDownstreamCacheProviders() : void
    {
        ...
        $cache1 = $this->getMockBuilder(ArrayCache::class)->setMethods(['doSave'])->getMock();
        $cache1->expects($this->once())->method('doSave')->with('[bar][1]', 'value', $defaultLifeTimeForDownstreamCacheProviders);
        $cache2 = new ArrayCache();
        $cache2->save('bar', 'value');
        $chainCache = new ChainCache([$cache1, $cache2]);
        $result = $chainCache->fetch('bar');
    ...
    public function testFetchPropagatesToFastestCacheUsingIndicatedDefaultLifeTimeForDownstreamCacheProviders() : void
    {
        ...
        $cache1 = $this->getMockBuilder(ArrayCache::class)->setMethods(['doSave'])->getMock();
        $cache1->expects($this->once())->method('doSave')->with('[bar][1]', 'value', $specificDefaultLifeTimeForDownstreamCacheProviders);
        $cache2 = new ArrayCache();
        $cache2->save('bar', 'value');
        $chainCache = new ChainCache([$cache1, $cache2]);
    ...
    {
        $specificDefaultLifeTimeForDownstreamCacheProviders = 12345;
        $cache1 = $this->getMockBuilder(ArrayCache::class)->setMethods(['doSaveMultiple'])->getMock();
        $cache1->expects($this->once())->method('doSaveMultiple')->with(['[bar][1]' => 'Bar', '[foo][1]' => 'Foo'], $specificDefaultLifeTimeForDownstreamCacheProviders);
        $cache2 = new ArrayCache();
        $cache2->saveMultiple(['bar' => 'Bar', 'foo' => 'Foo']);
        $chainCache = new ChainCache([$cache1, $cache2]);
        $chainCache->setDefaultLifeTimeForDownstreamCacheProviders($specificDefaultLifeTimeForDownstreamCacheProviders);
    ...
    {
        $cache1 = $this->getMockForAbstractClass('BuddyBossPlatform\\Doctrine\\Common\\Cache\\CacheProvider');
        $cache2 = $this->getMockForAbstractClass('BuddyBossPlatform\\Doctrine\\Common\\Cache\\CacheProvider');
    ...
    {
        $cache1 = $this->getMockBuilder(CacheProvider::class)->setMethods(['doDeleteMultiple'])->getMockForAbstractClass();
        $cache2 = $this->getMockBuilder(CacheProvider::class)->setMethods(['doDeleteMultiple'])->getMockForAbstractClass();
        $cache1->expects($this->once())->method('doDeleteMultiple')->willReturn(\true);
        $cache2->expects($this->once())->method('doDeleteMultiple')->willReturn(\true);
        $chainCache = new ChainCache([$cache1, $cache2]);
