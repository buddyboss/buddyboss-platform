...
namespace BuddyBossPlatform\ZipStream;

use HashContext;
use BuddyBossPlatform\Psr\Http\Message\StreamInterface;
use BuddyBossPlatform\ZipStream\Exception\FileNotFoundException;
use BuddyBossPlatform\ZipStream\Exception\FileNotReadableException;
use BuddyBossPlatform\ZipStream\Exception\OverflowException;
use BuddyBossPlatform\ZipStream\Option\File as FileOptions;
use BuddyBossPlatform\ZipStream\Option\Method;
use BuddyBossPlatform\ZipStream\Option\Version;
class File
{
    ...
    public function addFileHeader() : void
    {
        ...
        $fields = [
            ...
            ['v', $this->bits],
            // General purpose bit flags - data descriptor flag set
            ['v', $this->method->getValue()],
            // Compression method
            ['V', $time],
            // Timestamp (DOS Format)
            ['V', $this->crc],
            // CRC32 of data (0 -> moved to data descriptor footer)
            ['V', $this->zlen->getLowFF($force)],
            // Length of compressed data (forced to 0xFFFFFFFF for zero header)
            ['V', $this->len->getLowFF($force)],
            // Length of original data (forced to 0xFFFFFFFF for zero header)
            ['v', $nameLength],
            // Length of filename
            ['v', \strlen($footer)],
        ];
    ...
    protected function buildZip64ExtraBlock(bool $force = \false) : string
    {
        ...
        if ($this->bits & self::BIT_EFS_UTF8) {
            ...
            $fields[] = ['v', 0x5653];
            // Choose 'ZS' for proprietary usage
            $fields[] = ['v', 0x0];
            // zero length
        }
