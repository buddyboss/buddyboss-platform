...
class BP_Tests_Blogs_Cache extends BP_UnitTestCase {
	...
	public function test_bp_blogs_update_meta_cache() {
		...
		wp_cache_delete( $b1, 'bb_blog_meta' );
		wp_cache_delete( $b2, 'bb_blog_meta' );

		// Caches should be empty
		$this->assertFalse( wp_cache_get( $b1, 'bb_blog_meta' ) );
		$this->assertFalse( wp_cache_get( $b2, 'bb_blog_meta' ) );
		...
		$b1_found = wp_cache_get( $b1, 'bb_blog_meta' );
		foreach ( $b1_expected as $k => $v ) {
			$this->assertSame( $v, $b1_found[ $k ] );
		}

		$b2_found = wp_cache_get( $b2, 'bb_blog_meta' );
	...
	public function test_update_blog_details_should_purge_blogmeta_cache() {
		...
		$this->assertNotEmpty( wp_cache_get( $b1, 'bb_blog_meta' ) );
		...
		$this->assertEmpty( wp_cache_get( $b1, 'bb_blog_meta' ) );
	}
}
