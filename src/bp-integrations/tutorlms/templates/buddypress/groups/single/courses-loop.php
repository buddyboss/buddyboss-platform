...
$is_enrolled            = false;
$lesson_list            = tutorlms_get_lesson_list( get_the_ID(), array( 'num' => - 1 ) );
$current_user_id        = get_current_user_id();
$access_list            = tutorlms_get_course_meta_setting( $post->ID, 'course_access_list' );
$admin_enrolled         = TutorLMS_Settings_Section::get_section_setting( 'TutorLMS_Settings_Section_General_Admin_User', 'courses_autoenroll_admin_users' );
$user_course_has_access = sfwd_lms_has_access( get_the_ID(), $current_user_id );
$course_price           = trim( tutorlms_get_course_meta_setting( get_the_ID(), 'course_price' ) );
$course_price_type      = tutorlms_get_course_meta_setting( get_the_ID(), 'course_price_type' );
$paypal_settings        = TutorLMS_Settings_Section::get_section_settings_all( 'TutorLMS_Settings_Section_PayPal' );
$completed              = 0;
$total                  = false;
$class                  = '';
...
if ( empty( $course_id ) ) {
	$course_id = tutorlms_get_course_id();
}
...
if ( false === $total ) {
	$total = tutorlms_get_course_steps_count( $course_id );
}
    ...
    <div class="bb-cover-list-item <?php echo $class; ?>">
        <div class="bb-course-cover">
            <a class="ld-set-cookie bb-cover-wrap" data-course-id="<?php echo esc_attr( get_the_ID() ); ?>" data-group-id="<?php echo esc_attr( ( bp_is_group_single() ? bp_get_current_group_id() : '' ) ); ?>" title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>" class="bb-cover-wrap">
				<?php
				$progress = tutorlms_course_progress( array(
				...
				if ( is_user_logged_in() && isset( $user_course_has_access ) && $user_course_has_access ) {
					...
					if ( ( isset( $course_pricing['type'] ) && $course_pricing['type'] === 'open' && isset( $progress['percentage'] ) && $progress['percentage'] === 0 ) || ( isset( $course_pricing['type'] ) && $course_pricing['type'] !== 'open' && $user_course_has_access && isset( $progress['percentage'] ) && $progress['percentage'] === 0 ) ) {
						...
						echo '<div class="ld-status ld-status-progress ld-primary-background">' . __( 'Start ', 'buddyboss' ) . sprintf( __( '%s', 'buddyboss' ), TutorLMS_Custom_Label::get_label( 'course' ) ) . '</div>';

					} else {

						tutorlms_status_bubble( $status );
        ...
        <div class="bb-card-course-details">
			...
			$lessons_count > 1
				...
				$lessons_count,
				TutorLMS_Custom_Label::get_label( 'lessons' )
			)
				: sprintf(
				__( '%1$s %2$s', 'buddyboss' ),
				$lessons_count,
				TutorLMS_Custom_Label::get_label( 'lesson' )
			)
			);
			...
			if ( $lessons_count > 0 ) {
				...
				echo '<div class="course-lesson-count">' . sprintf( __( '0 %s', 'buddyboss' ), TutorLMS_Custom_Label::get_label( 'lessons' ) ) . '</div>';
			}
			?>
			...
			<?php
			if ( buddyboss_theme_get_option( 'tutorlms_course_author' ) ) {
			...
			if ( is_user_logged_in() && isset( $user_course_has_access ) && $user_course_has_access ) { ?>
                ...
                <div class="course-progress-wrap">
					...
					<?php tutorlms_get_template_part( 'modules/progress.php',
						array(
							'context'   => 'course',
							'user_id'   => $current_user_id,
							'course_id' => $course_id,
			...
			if ( ! empty( $course_price ) && empty( $is_enrolled ) ) { ?>
                ...
                <span class="course-fee">
                        ...
                        if ( $course_pricing['type'] !== 'closed' ):
	                        echo wp_kses_post( '<span class="ld-currency">' . tutorlms_30_get_currency_symbol() . '</span> ' );
                        endif;
