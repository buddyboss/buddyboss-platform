...
$ld_group_id = bb_tutorlms( 'buddypress' )->helpers->getLearndashGroupId( bp_get_current_group_id() );

if ( $ld_group_id ) {
	$post_label_prefix = 'group';
	$meta              = tutorlms_get_setting( $ld_group_id );
	...
	if ( ! empty( $post_price ) && ! tutorlms_is_user_in_group( bp_loggedin_user_id(), $ld_group_id ) ) {
		?>
...
$course_settings            = tutorlms_get_setting( $course );
$lesson_progression_enabled = tutorlms_lesson_progression_enabled( $course_id );
$courses_options            = tutorlms_get_option( 'sfwd-courses' );
$lessons_options            = tutorlms_get_option( 'sfwd-lessons' );
$quizzes_options            = tutorlms_get_option( 'sfwd-quiz' );
$course_status              = tutorlms_course_status( $course_id, null );
...
$lessons                    = tutorlms_get_course_lessons_list( $course, $user_id );
$quizzes                    = tutorlms_get_course_quiz_list( $course );
$has_course_content         = ( ! empty( $lessons ) || ! empty( $quizzes ) );
$lesson_query_args          = tutorlms_focus_mode_lesson_query_args( $course_id );
$lessons                    = tutorlms_30_get_course_navigation( $course_id, array(), $lesson_query_args );
...
if ( ! empty( $lessons ) ) {
	foreach ( $lessons as $lesson ) {
		...
		$all_topics = tutorlms_topic_dots( $lesson['post']->ID, false, 'array', null, $course_id );
		...
		$lesson_topics[ $lesson['post']->ID ] = tutorlms_process_lesson_topics_pager( $all_topics, $topic_pager_args );
...
$course_certficate_link = tutorlms_get_course_certificate_link( $course_id, $user_id );
...
$has_lesson_quizzes = tutorlms_30_has_lesson_quizzes( $course_id, $lessons );
?>



<div class="<?php echo esc_attr( tutorlms_the_wrapper_class() ); ?>">
	 ...
	 * @since 3.0
	 */
	do_action( 'tutorlms-course-before', $course_id, $course_id, $user_id );
	 ...
	 * @since 3.0
	 */
	do_action( 'tutorlms-course-certificate-link-before', $course_id, $user_id );
	 ...
	 * @since 3.0
	 */
	do_action( 'tutorlms-course-certificate-link-after', $course_id, $user_id );
	?>

	<div class="bb-grid">

		<div class="bb-tutorlms-content-wrap">
			 ...
			 *
			 * @since 2.3
			 * See https://bitbucket.org/snippets/tutorlms/7oe9K for example use of this filter.
			 */
			echo apply_filters(
				'ld_after_course_status_template_container',
				'',
				tutorlms_course_status_idx( $course_status ),
				$course_id,
				$user_id
			);
			...
			echo '<div id="tutorlms-course-content"></div>';
			tutorlms_get_template_part(
				'modules/tabs.php',
				array(
					'course_id' => $course_id,
			...
			if ( $has_course_content && $show_course_content ) :
				...
				<div class="ld-item-list ld-lesson-list">
					<div class="ld-section-heading">
						 ...
						 * @since 3.0
						 */
						do_action( 'tutorlms-course-heading-before', $course_id, $user_id );
						?>
						...
						printf(
							...
							esc_attr( TutorLMS_Custom_Label::get_label( 'course' ) )
						);
						?>
							</h2>
						 ...
						 * @since 3.0
						 */
						do_action( 'tutorlms-course-heading-after', $course_id, $user_id );
						?>
						...
						<div class="ld-item-list-actions" data-ld-expand-list="true">
							 ...
							 * @since 3.0
							 */
							do_action( 'tutorlms-course-expand-before', $course_id, $user_id );
							?>
							...
							if ( $has_topics || $has_lesson_quizzes ) :
								...
								if ( apply_filters(
									'tutorlms_course_steps_expand_all',
									false,
									$course_id,
							 ...
							 * @since 3.0
							 */
							do_action( 'tutorlms-course-expand-after', $course_id, $user_id );
							?>
					 ...
					 * @since 3.0
					 */
					do_action( 'tutorlms-course-content-list-before', $course_id, $user_id );
					 ...
					 * @since 3.0
					 */
					do_action( 'tutorlms-course-content-list-after', $course_id, $user_id );
					?>
		...
		$post = $courses_new[0];
		tutorlms_get_template_part( 'template-single-course-sidebar.php', $template_args, true );
		?>
	</div>
	 ...
	 * @since 3.0
	 */
	do_action( 'tutorlms-course-after', $course_id, $course_id, $user_id );

	tutorlms_get_template_part( 'modules/login-modal.php', array(), true );
	?>
