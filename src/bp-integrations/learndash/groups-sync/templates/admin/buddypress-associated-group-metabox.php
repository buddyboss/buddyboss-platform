<?php
    ...
    $post_settings   = groups_get_groupmeta($group_id, 'bp_learndash_groups_sync_settings', true) ?: [];
...
<?php if ($ld_group = bp_learndash_groups_sync_get_associated_ld_group($group_id)): ?>
    <p>
...
<?php else: ?>
    ...
    <select name="bp_learndash_groups_sync[learndash_group_id]" style="width: 100%; margin-bottom: 10px;">
        <option value="0"><?php _e('None', 'buddyboss'); ?></option>

        <?php if (! bp_learndash_groups_sync_get_unassociated_ld_groups([], $ld_group)): ?>
            <option disabled><?php _e('No LearnDash group available.'); ?></option>
        <?php endif; ?>

        <?php foreach (bp_learndash_groups_sync_get_unassociated_ld_groups([], $ld_group) as $group): ?>
