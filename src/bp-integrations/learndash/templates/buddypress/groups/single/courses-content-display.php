...
if ( ! empty( $lessons ) ) {
	foreach ( $lessons as $lesson ) {
		...
		$topic_pager_args = apply_filters( 'ld30_ajax_topic_pager_args',
			array(
				'course_id' => $course_id,
...
<div class="<?php echo esc_attr( learndash_the_wrapper_class() ); ?>">
	...
	if ( $course_certficate_link && ! empty( $course_certficate_link ) ):
		...
		learndash_get_template_part( 'modules/alert.php',
			array(
	...
	<div class="bb-grid">
		...
		<div class="bb-learndash-content-wrap">
			 ...
			 *
			 */
			learndash_get_template_part( 'modules/infobar.php',
				array(
					'context'       => 'course',
					'course_id'     => $course_id,
					'user_id'       => $user_id,
			 ...
			 */
			echo apply_filters( 'ld_after_course_status_template_container',
				'',
			...
			learndash_get_template_part( 'modules/tabs.php',
				array(
					'course_id' => $course_id,
					'post_id'   => $course_id,
					'user_id'   => $user_id,
			...
			if ( $has_course_content && $show_course_content ): ?>
				...
				<div class="ld-item-list ld-lesson-list">
					<div class="ld-section-heading">
						...
						<h2><?php printf( esc_html_x( '%s Content', 'Course Content Label', 'buddyboss' ),
								esc_attr( LearnDash_Custom_Label::get_label( 'course' ) ) ); ?></h2>
						...
						<div class="ld-item-list-actions" data-ld-expand-list="true">
							...
							if ( $has_topics || $has_lesson_quizzes ): ?>
								<div class="ld-expand-button ld-primary-background" id="<?php echo esc_attr( 'ld-expand-button-' . $course_id ); ?>" data-ld-expands="<?php echo esc_attr( 'ld-item-list-' . $course_id ); ?>" data-ld-expand-text="<?php echo esc_attr_e( 'Expand All',
									'buddyboss' ); ?>" data-ld-collapse-text="<?php echo esc_attr_e( 'Collapse All',
									'buddyboss' ); ?>">
									<span class="ld-icon-arrow-down ld-icon"></span>
									<span class="ld-text"><?php echo esc_html_e( 'Expand All', 'buddyboss' ); ?></span>
								</div> <!--/.ld-expand-button-->							<?php
							// TODO @37designs Need to test this
							if ( apply_filters( 'learndash_course_steps_expand_all',
								false,
								$course_id,
					...
					learndash_get_template_part( 'course/listing.php',
						array(
							'course_id'                  => $course_id,
							'user_id'                    => $user_id,
							'lessons'                    => $lessons,
			...
			learndash_get_template_part( 'template-course-author-details.php',
				array(
					'context'   => 'course',
					'course_id' => $course_id,
					'user_id'   => $user_id,
		...
		<div class="bb-single-course-sidebar bb-preview-wrap">
			<div class="widget bb-enroll-widget">
							...
							<div class="thumbnail-container <?php echo $thumb_mode; ?>">
								<div class="bb-course-video-overlay">
									<div>
										...
										<div><?php _e( 'Preview this',
												'buddyboss' ); ?><?php echo LearnDash_Custom_Label::get_label( 'course' ) ?></div>
									</div>
								</div>
				...
				<div class="bb-course-preview-content">
					...
					<?php foreach ( $course_members as $course_member ) : ?><?php if ( $count > 2 ) {
						break;
					} ?>
						<img class="round" src="<?php echo get_avatar_url( $course_member->ID,
							array( 'size' => 96 ) ); ?>" alt=""/>
						<?php $count ++; endforeach; ?>
					</span>

							<?php if ( sizeof( $course_members ) > 3 ) { ?>
							<span class="members"><span class="members-count-g">+<?php echo sizeof( $course_members ) - 3; ?></span><?php } ?> <?php _e( 'enrolled',
									'buddyboss' ); ?></span>
						</div>
					<?php } ?>
					...
					<div class="bb-course-status-wrap">
						...
						<?php do_action( 'learndash-course-infobar-status-cell-before',
							get_post_type(),
							$course_id,
							$current_user_id ); ?>
						...
						<?php do_action( 'learndash-course-infobar-status-cell-after',
							get_post_type(),
							$course_id,
							$current_user_id ); ?>
					...
					<div class="bb-button-wrap">
						...
						if ( empty( $course_progress ) && $course_progress < 100 ) {
							...
							$btn_advance_label = __( 'Start Course', 'buddyboss' );
						...
						$login_model = LearnDash_Settings_Section::get_section_setting( 'LearnDash_Settings_Theme_LD30',
							'login_mode_enabled' );
						$login_url   = apply_filters( 'learndash_login_url',
							( $login_model === 'yes' ? '#login' : wp_login_url( get_the_permalink( $course_id ) ) ) );
						...
						if ( $course_price_type == 'open' || $course_price_type == 'free' ) {
							...
							if ( apply_filters( 'learndash_login_modal',
								...
								<div class="learndash_join_button <?php echo $btn_advance_class; ?>">
									<a href="<?php echo esc_url( $login_url ); ?>" class="btn-advance"><?php echo __( 'Login to Enroll',
											'buddyboss' ); ?></a>
								</div>
							<?php
							...
							if ( $course_price_type == 'open' ) {
								?><span class="bb-course-type bb-course-type-open"><?php _e( 'Open Registration',
									'buddyboss' ); ?></span><?php
							} else {
								?><span class="bb-course-type bb-course-type-free"><?php _e( 'Free',
									'buddyboss' ); ?></span><?php
							}
							...
							if ( empty( $learndash_payment_buttons ) ):
								echo '<span class="ld-status ld-status-incomplete ld-third-background ld-text">' . __( 'This course is currently closed',
										'buddyboss' ) . '</span>';
							else:
								?>
							...
							if ( apply_filters( 'learndash_login_modal',
								...
								echo '<span class="ld-status">' . __( 'or ',
										'buddyboss' ) . '<a class="ld-login-text" href="' . esc_attr( $login_url ) . '">' . __( 'Login',
										'buddyboss' ) . '</a></span>';
							endif;
							...
							if ( $course_price_type == 'paynow' ) {
								...
								?>
								<span class="bb-course-type bb-course-type-paynow"><?php echo bp_ld_sync()->bp_ld_prepare_price_str( array(
						    ...
						    if ( '' === $course_price && $course_price_type == 'subscribe' ) {
							    ?>
							    <span class="bb-course-type bb-course-type-subscribe"><?php _e( 'Free',
									    'buddyboss' ); ?></span>
							    <?php
						    } else {
							    echo bp_ld_sync()->bp_ld_prepare_price_str( array(
								    'code'  => $paypal_settings['paypal_currency'],
								    'value' => $course_price,
							    ) );
						    }
						...
						<div class="bb-course-volume">
							...
							<ul class="bb-course-volume-list">
								<?php if ( sizeof( $lessons ) > 0 ) { ?>
									<li>
										<i class="bb-icons bb-icon-book"></i><?php echo sizeof( $lessons ); ?> <?php echo sizeof( $lessons ) > 1 ? LearnDash_Custom_Label::get_label( 'lessons' ) : LearnDash_Custom_Label::get_label( 'lesson' ); ?>
									</li>
								<?php } ?>
