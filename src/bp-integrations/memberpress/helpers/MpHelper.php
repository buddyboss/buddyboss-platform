<?php
namespace BuddyBoss\Integrations\MemberPress\Helpers;

use BuddyBoss\Integrations\BbmsHelper;
use BuddyBoss\Integrations\Includes\BbmsView;
...
class MpHelper {
	...
	public static function mpLearndashTabContent($product) {
		...
		$allCourses = BbmsHelper::getLearndashCourses();
		...
		$accessMethods = BbmsHelper::getCourseOptions();
		$groups = array();
		...
		// BbmsView::render('/memberpress/mp-tab-content', get_defined_vars());
	...
	public static function mpSubscriptionUpdated($meprObj) {
		...
		if (in_array($status, $revokeValues)) {
			// revoke access
			BbmsHelper::bbmsUpdateMembershipAccess($meprObj, MP_POST_TYPE, false);
		} else if (in_array($status, $grantValues)) {
			// grant access
			BbmsHelper::bbmsUpdateMembershipAccess($meprObj, MP_POST_TYPE, true);
		}
	...
	public static function mpSaveProduct($product) {
		...
		if ($isEnabled) {
			...
			if ($courseAccessMethod == 'SINGLE_COURSES') {
				...
				$allClosedCourses = BbmsHelper::getLearndashClosedCourses();
