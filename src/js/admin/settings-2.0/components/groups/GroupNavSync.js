 ...
 */
export function useGroupNavSync( {
	featureId,
	settings,
	settingsRef,
	initialLoad,
	...
	useEffect( function () {
		...
		var navOrderValue = settings.bb_group_nav_order;
					...
					sections: panel.sections.map( function ( section ) {
						return Object.assign( {}, section, {
							fields: section.fields.map( function ( field ) {
								if ( 'bb_group_default_tab' !== field.name ) {
									return field;
								}
		...
		var currentDefault = settingsRef.current.bb_group_default_tab;
				...
				if ( hiddenSlugs.indexOf( allOpts[ j ].value ) === -1 ) {
					if ( initialLoad ) {
						...
						setSettings( function ( prev ) {
							return Object.assign( {}, prev, { bb_group_default_tab: allOpts[ j ].value } );
						} );
					} else {
						// User interaction — trigger save.
						handleSettingChange( 'bb_group_default_tab', allOpts[ j ].value );
					}
					break;
				}
			}
		}
	}, [ featureId, settings.bb_group_nav_order, initialLoad, handleSettingChange, setSidePanels, setSettings, settingsRef ] );
}
