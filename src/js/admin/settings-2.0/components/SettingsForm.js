			...
			case 'toggle':
				...
				return (
					<div className="bb-admin-settings-form__toggle-wrapper">
						<ToggleControl
							key={field.name}
							label={ field.toggle_label ? decodeEntities( field.toggle_label ) : ( field.description_controls && field.description_controls.length > 0 ? '' : decodeEntities( field.description || '' ) ) }
		...
		return (
			<div key={field.name} className={fieldClasses + ( ! hasLabel ? ' bb-admin-settings-form__field--no-label' : '' ) + ( 'reaction_mode' !== field.type && field.pro_notice?.show ? ' bb-admin-settings-form__field--pro-locked' : '' )} data-group={field.group?.key || undefined}>
				...
				<div className={ 'bb-admin-settings-form__field-content' + ( ( 'toggle' === field.type || 'checkbox' === field.type ) && field.description && ! isToggleWithChildren && ! field.block_description ? ' bb-admin-settings-form__field-content--inline' : '' ) }>
					...
					{ field.description && -1 === [ 'notice', 'checkbox_list', 'share_platforms', 'topic_list' ].indexOf( field.type ) && ( 'toggle' !== field.type || field.block_description || ( field.description_controls && field.description_controls.length > 0 ) ) && ( () => {
