...
function groups_screen_group_messages() {
	...
	if ( bp_is_action_variable( 'send', 0 ) ) {
		 ...
		 */
		do_action( 'groups_screen_group_messages', $bp->groups->current_group->id );
		bp_core_redirect( bp_get_group_permalink( $bp->groups->current_group ) );

	} elseif ( !bp_action_variable( 0 ) ) {

		/**
		 * Filters the template to load for a group's Group Messages page.
		 *
		 * @since BuddyPress 1.0.0
		 *
		 * @param string $value Path to a group's Group Messages template.
		 */
