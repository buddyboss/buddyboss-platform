...
class BB_Icon_Registry {
	 ...
	 */
	public function bb_register_icon( $icon_id, $args = array() ) {
		// Validate icon ID.
		...
		$validation_error = $this->bb_validate_icon_args( $icon_id, $type, $args );
	 ...
	 */
	private function bb_validate_icon_args( $icon_id, $type, $args ) {
		switch ( $type ) {
			case 'dashicon':
				if ( empty( $args['slug'] ) ) {
					return new WP_Error(
	 ...
	 */
	public function bb_get_icon( $icon_id ) {
	 ...
	 */
	public function bb_get_icons( $type = null ) {
		$icons = $this->icons;

		if ( ! is_null( $type ) ) {
			$icons = array_filter(
				$icons,
				function ( $icon ) use ( $type ) {
	 ...
	 */
	public function bb_is_registered( $icon_id ) {
	 ...
	 */
	public function bb_get_icon_for_rest( $icon_id ) {
		...
		if ( $this->bb_is_registered( $icon_id ) ) {
			$icon = $this->bb_get_icon( $icon_id );
			return array(
				'type' => $icon['type'],
				'id'   => $icon_id,
				'data' => $icon,
		...
		if ( $this->bb_is_dashicon( $icon_id ) ) {
			return array(
				'type' => 'dashicon',
				'slug' => $icon_id,
			);
	 ...
	 */
	private function bb_is_dashicon( $icon_id ) {
	 ...
	 */
	private function bb_is_url( $icon_id ) {
