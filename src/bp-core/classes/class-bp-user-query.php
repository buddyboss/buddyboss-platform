	...
	public function prepare_user_ids_query() {
		...
		switch ( $type ) {
			...
			case 'online':
				 ...
				 */
				$sql['where'][] = $wpdb->prepare( 'u.date_recorded >= DATE_SUB( UTC_TIMESTAMP(), INTERVAL %d MINUTE )', apply_filters( 'bp_user_query_online_interval', ( bb_heartbeat_interval() + 5 ) / 60 ) );
