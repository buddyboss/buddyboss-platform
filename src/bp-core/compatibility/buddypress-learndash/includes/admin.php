...
if ( ! class_exists( 'BuddyPress_LearnDash_Admin' ) ):
	...
	class BuddyPress_LearnDash_Admin {
		...
		public function admin_init() {
			...
			add_settings_section( 'general_section', __( 'General Settings', 'buddyboss' ), array(
				$this,
				'section_general'
			), __FILE__ );

			add_settings_field( 'courses_visibility_option', __( 'Visibility', 'buddyboss' ), array(
				$this,
				'courses_visibility_option'
			), __FILE__, 'general_section' );
			add_settings_field( 'convert_subscribers_option', __( 'User Roles', 'buddyboss' ), array(
				$this,
				'convert_subscribers_option'
			), __FILE__, 'general_section' );
			add_settings_field( 'convert_group_leaders_option', '', array(
				$this,
				'convert_group_leaders_option'
			), __FILE__, 'general_section' );
		}
		...
		public function admin_menu() {
			add_submenu_page(
				$this->settings_page, 'BuddyPress for LearnDash', 'BuddyPress for LearnDash', $this->capability, $this->plugin_slug, array(
					$this,
					'options_page'
				)
			);
		...
		function plugin_settings_link( $links ) {
			$settings_link = '<a href="' . admin_url( "options-general.php?page=" . __FILE__ ) . '">' . __( 'Settings', 'buddyboss' ) . '</a>';
		...
		public function options_page() {
			?>
            <div class="wrap">
                <h2><?php _e( 'Buddypress Learndash', 'buddyboss' ); ?></h2>
                <form action="<?php echo $this->form_action; ?>" method="post">
                    ...
                    <p class="submit">
                        <input name="bp_learndash_settings_submit" type="submit" class="button-primary"
                               value="<?php esc_attr_e( 'Save Changes', 'buddyboss' ); ?>"/>
                    </p>
                </form>
            </div>
		...
		public function courses_visibility_option() {
			...
			printf( __( 'Display <em>Profile > %s</em> content publicly', 'buddyboss' ), LearnDash_Custom_Label::get_label( 'course' ) );
		}
		...
		public function convert_subscribers_option() {
			...
			_e( 'Convert subscribers to user role Student', 'buddyboss' );
		}
		...
		public function convert_group_leaders_option() {
			...
			_e( 'Convert group leaders to user role Group Leader', 'buddyboss' );
		}
