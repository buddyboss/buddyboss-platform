...
function bp_core_change_privacy_policy_link_on_private_network( $link, $privacy_policy_url ) {
	...
	if ( ! is_user_logged_in() ) {
		...
		if ( $privacy_policy_url && $page_title ) {
			...
			$get_content        = apply_filters( 'bp_privacy_policy_content', wp_kses_post( bp_strip_script_and_style_tags( apply_filters( 'the_content', $get_privacy_policy->post_content ) ) ), $get_privacy_policy->post_content );
	...
	if (
		...
		if ( ! empty( $terms ) && ! empty( $privacy ) ) {
			...
			$get_term_content = apply_filters( 'bp_term_of_service_content', wp_kses_post( bp_strip_script_and_style_tags( apply_filters( 'the_content', $get_terms->post_content ) ) ), $get_terms->post_content );
			$link             = sprintf(
			...
			$get_content      = apply_filters( 'bp_privacy_policy_content', wp_kses_post( bp_strip_script_and_style_tags( apply_filters( 'the_content', $get_privacy->post_content ) ) ), $get_privacy->post_content );
