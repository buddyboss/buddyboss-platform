...
function bb_post_new_comment_reply_notification( $comment_id, $comment_approved, $commentdata ) {
	...
	$comment_author_name      = ! empty( $comment_author ) ? bp_core_get_user_displayname( $comment_author->ID, $parent_comment_author_id, false ) : $commentdata['comment_author'];
...
function bb_mention_post_type_comment( $comment_id = 0, $is_approved = true ) {
	...
	foreach ( (array) $usernames as $user_id => $username ) {
		...
		if (
			...
			// Poster name.
			$reply_author_name = bp_core_get_user_displayname( $comment_user_id, $user_id, false );
