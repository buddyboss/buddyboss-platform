			...
			if ( 'undefined' === typeof bb_livestamp.chunks[ i ] ) {
				...
				switch ( seconds ) {
					case parseInt( bb_livestamp.year_in_seconds ):
						output = ( count < 2 ) ? bb_livestamp.year_text : bbConcateString( count, bb_livestamp.years_text );
						break;
					case ( parseInt( bb_livestamp.year_in_seconds ) / 6 ):
						var month_seconds = Math.floor( since / ( 30 * parseInt( bb_livestamp.day_in_seconds ) ) );

						output = ( month_seconds < 2 ) ? bb_livestamp.month_text : bbConcateString( month_seconds, bb_livestamp.months_text );
						break;
					case ( 30 * parseInt( bb_livestamp.day_in_seconds ) ):
						var week_seconds = Math.floor( since / parseInt( bb_livestamp.week_in_seconds ) );

						if ( count < 2 ) {
							output = ( week_seconds < 2 ) ? bb_livestamp.week_text : bbConcateString( week_seconds, bb_livestamp.weeks_text );
						} else {
							output = ( count < 2 ) ? bb_livestamp.month_text : bbConcateString( floor_count, bb_livestamp.months_text );
						}
						break;
					case parseInt( bb_livestamp.week_in_seconds ):
						output = ( count < 2 ) ? bb_livestamp.week_text : bbConcateString( floor_count, bb_livestamp.weeks_text );
						break;
					case parseInt( bb_livestamp.day_in_seconds ):
						output = ( count < 2 ) ? bb_livestamp.day_text : bbConcateString( floor_count, bb_livestamp.days_text );
						break;
					case parseInt( bb_livestamp.hour_in_seconds ):
						output = ( count < 2 ) ? bb_livestamp.hour_text : bbConcateString( floor_count, bb_livestamp.hours_text );
						break;
					case parseInt( bb_livestamp.minute_in_seconds ):
						output = ( count < 2 ) ? bb_livestamp.minute_text : bbConcateString( floor_count, bb_livestamp.minutes_text );
						break;
					default:
						output = ( count < 2 ) ? bb_livestamp.second_text : bbConcateString( floor_count, bb_livestamp.seconds_text );
				}
