...
function bb_admin_setting_callback_activity_comment_sorting() {
	...
	<div class="bb-activity-sorting-list">
		<?php
		$sorting_options_labels = bb_get_activity_comment_sorting_options_labels();

		// Retrieve the saved options.
		$sorting_options = bb_get_enabled_activity_comment_sorting_options();
		if ( ! empty( $sorting_options ) ) {
			// Sort filter labels based on the order of $sorting_options.
			$sorted_labels = array();
			foreach ( $sorting_options as $key => $value ) {
		...
		foreach ( $sorted_labels as $key => $label ) :
			...
			<div class="bb-activity-sorting-item">
				<input
					type="hidden"
					name="bb_activity_comment_sorting_options[<?php echo esc_attr( $key ); ?>]"
					value="<?php echo ( 'oldest_first' === $key ) ? 1 : 0 ?>"
					<?php echo isset( $sorting_options[ $key ] ) && ! empty( (bool) $sorting_options[ $key ] && 'oldest_first' !== $key ) ? 'disabled' : ''; ?>
				/>
				<input
					<?php echo $readonly; ?>
					id="<?php echo esc_attr( $key ); ?>" 
