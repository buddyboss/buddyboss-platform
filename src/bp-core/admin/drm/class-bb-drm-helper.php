...
class BB_DRM_Helper {
	...
	protected static function drm_info_no_license( $drm_status, $purpose ) {
		...
		$additional_instructions = sprintf(
			...
			__( 'This is an automated message from %s.', 'buddyboss-platform' ),
			esc_url( home_url() )
		);
		...
		switch ( $drm_status ) {
			case self::DRM_LOW:
				...
				$heading           = __( 'License Activation Needed', 'buddyboss-platform' );
				$color             = 'FFA500'; // Orange.
				$simple_message    = __( 'We couldn\'t verify an active license for your BuddyBoss paid add-on features. Please activate your license to continue using them.', 'buddyboss-platform' );
				$help_message      = __( 'Activate Your License', 'buddyboss-platform' );
				...
				$message           = sprintf(
					...
					sprintf(
						...
						__( 'Need your license key? Visit %1$sYour Account Page%2$s. Having trouble? %3$sContact Support%2$s.', 'buddyboss-platform' ),
			...
			case self::DRM_MEDIUM:
				...
				$heading           = __( 'License Required', 'buddyboss-platform' );
				$color             = 'FFA500'; // Yellow/Orange.
				$simple_message    = __( 'An active license is required to use BuddyBoss Pro/Plus add-on features. Without activation, these add-on features will stop working.', 'buddyboss-platform' );
				$help_message      = __( 'Activate Your License', 'buddyboss-platform' );
				$label             = __( 'Warning', 'buddyboss-platform' );
				$activation_link   = bp_get_admin_url( 'admin.php?page=buddyboss-license' );
				$message           = sprintf(
					'<p>%s</p><p>%s</p>',
					$simple_message,
			...
			case self::DRM_HIGH:
				...
				$heading           = __( 'License Activation Required', 'buddyboss-platform' );
				$color             = 'FF8C00'; // Dark Orange.
				$simple_message    = __( 'Your BuddyBoss Pro/Plus add-on features will be disabled soon. Activate your license now to avoid interruption.', 'buddyboss-platform' );
				$help_message      = __( 'Activate Your License', 'buddyboss-platform' );
				$label             = __( 'Critical', 'buddyboss-platform' );
				$activation_link   = bp_get_admin_url( 'admin.php?page=buddyboss-license' );
				$message           = sprintf(
					'<p>%s</p><p>%s</p>',
					$simple_message,
			...
			case self::DRM_LOCKED:
				...
				$heading           = __( 'BuddyBoss Pro/Plus Add-on Features Disabled', 'buddyboss-platform' );
				$color             = 'dc3232'; // Red.
				$simple_message    = __( ' The following paid add-on features have been disabled because no active license was found. Activate your license to restore them.', 'buddyboss-platform' );
				$help_message      = __( 'Activate Your License', 'buddyboss-platform' );
				$label             = __( 'Critical', 'buddyboss-platform' );
				$activation_link   = bp_get_admin_url( 'admin.php?page=buddyboss-license' );
				$message           = sprintf(
					'<p>%s</p><p>%s</p><p>%s</p>',
					$simple_message,
					__( 'If you no longer need these features, you can deactivate the premium add-ons in your Plugins page to continue using BuddyBoss Platform for free.', 'buddyboss-platform' ),
					sprintf(
						/* translators: %1$s: opening anchor tag for account page, %2$s: closing anchor tag, %3$s: opening anchor tag for support */
						__( 'Need your license key? Visit %1$sYour Account Page%2$s. Having trouble? %3$sContact Support%2$s.', 'buddyboss-platform' ),
						'<a href="' . esc_url( $account_link ) . '" target="_blank">',
	...
	protected static function drm_info_invalid_license( $drm_status, $purpose ) {
		...
		$additional_instructions = sprintf(
			...
			__( 'This is an automated message from %1$s. If you continue getting these messages, please try deactivating and then re-activating your license key on %2$s.', 'buddyboss-platform' ),
			esc_url( home_url() ),
			esc_url( home_url() )
		);
		...
		switch ( $drm_status ) {
			case self::DRM_LOW:
				// 7-13 days: Plugin Notification only (per BuddyBoss DRM Messaging.md)
				$admin_notice_view = 'low_notification';
				$heading           = __( 'BuddyBoss Launch/Scale: License Activation Needed', 'buddyboss-platform' );
				$color             = 'FFA500'; // Orange.
				$simple_message    = __( 'We couldn\'t verify an active license for your BuddyBoss Launch/Scale features. Please activate your license to continue using them.', 'buddyboss-platform' );
				$help_message      = __( 'Activate Your License', 'buddyboss-platform' );
				$label             = __( 'Notice', 'buddyboss-platform' );
				$activation_link   = bp_get_admin_url( 'admin.php?page=buddyboss-license' );
				$message           = sprintf(
			...
			case self::DRM_MEDIUM:
				// 14-21 days: Admin Notice (Yellow) + Plugin Notification + Site Health (per BuddyBoss DRM Messaging.md)
				$admin_notice_view = 'medium_warning';
				$heading           = __( 'BuddyBoss Launch/Scale: License Required', 'buddyboss-platform' );
				$color             = 'FFA500'; // Yellow/Orange.
				$simple_message    = __( 'An active license is required to use BuddyBoss Launch/Scale features. Without activation, these features will stop working.', 'buddyboss-platform' );
				$help_message      = __( 'Activate Your License', 'buddyboss-platform' );
				$label             = __( 'Warning', 'buddyboss-platform' );
				$activation_link   = bp_get_admin_url( 'admin.php?page=buddyboss-license' );
				$message           = sprintf(
			...
			case self::DRM_HIGH:
				// 21-30 days: Admin Notice (Orange) + Plugin Notification + Site Health + Email (per BuddyBoss DRM Messaging.md)
				$admin_notice_view = 'high_warning';
				$heading           = __( 'BuddyBoss Launch/Scale: Activation Required', 'buddyboss-platform' );
				$color             = 'FF8C00'; // Dark Orange.
				$simple_message    = __( 'Your BuddyBoss Launch/Scale features will be disabled soon. Activate your license now to avoid interruption.', 'buddyboss-platform' );
				$help_message      = __( 'Activate Your License', 'buddyboss-platform' );
				$label             = __( 'Critical', 'buddyboss-platform' );
				$activation_link   = bp_get_admin_url( 'admin.php?page=buddyboss-license' );
				$message           = sprintf(
			...
			case self::DRM_LOCKED:
				...
				$admin_notice_view = 'locked_warning';
				$label             = __( 'Critical', 'buddyboss-platform' );
				$heading           = __( 'BuddyBoss Launch/Scale: Features Disabled', 'buddyboss-platform' );
				$color             = 'dc3232'; // Red.
				$simple_message    = __( 'The following features have been disabled because no active license was found. Activate your license to restore them.', 'buddyboss-platform' );
				$activation_link   = bp_get_admin_url( 'admin.php?page=buddyboss-license' );
				$message           = sprintf(
					'<p>%s</p><p>%s</p><p>%s</p>',
					$simple_message,
