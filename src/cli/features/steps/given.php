...
$steps->Given(
	'/^an? (empty|non-existent) ([^\s]+) directory$/',
...
$steps->Given(
	'/^"([^"]+)" replaced with "([^"]+)" in the ([^\s]+) file$/',
	function( $world, $search, $replace, $path ) {
		$full_path = $world->variables['RUN_DIR'] . "/$path";
...
$steps->Given(
	"/^a WP install with Composer and a custom vendor directory '([^\s]+)'$/",
...
$steps->Given(
	'/^a WP multisite (subdirectory|subdomain)?\s?install$/',
...
$steps->Given(
	'/^a new Phar with (?:the same version|version "([^"]+)")$/',
...
$steps->Given(
	'/^a downloaded Phar with (?:the same version|version "([^"]+)")$/',
	function ( $world, $version = 'same' ) {
...
$steps->Given(
	'/^a misconfigured WP_CONTENT_DIR constant directory$/',
	function( $world ) {
		$wp_config_path = $world->variables['RUN_DIR'] . '/wp-config.php';
...
$steps->Given(
	'/^a dependency on current wp-cli$/',
	function ( $world ) {
