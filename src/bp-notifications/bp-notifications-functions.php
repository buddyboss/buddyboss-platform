	...
	foreach ( $notifications as $notification_item ) {
		...
		if ( isset( $bp->{$component_name}->notification_callback ) && is_callable( $bp->{$component_name}->notification_callback ) ) {
			...
			if ( 'object' === $format ) {
				...
				$content = apply_filters( 'bb_notifications_get_component_notification', $content, $notification_item->component_action, $notification_item->item_id, $notification_item->secondary_item_id, $notification_item->total_count, 'array', $notification_item->id, 'web' );
				...
				$content = apply_filters( 'bb_notifications_get_component_notification', $content, $notification_item->component_action, $notification_item->item_id, $notification_item->secondary_item_id, $notification_item->total_count, 'string', $notification_item->id, 'web' );
