...
function bbp_save_forum_extras( $forum_id = 0 ) {
	...
	/** Forum Visibility */
	if ( ! bb_forum_visibility_dropdown_disabled( $forum_id ) && ! empty( $_POST['bbp_forum_visibility'] ) && in_array( $_POST['bbp_forum_visibility'], array( bbp_get_public_status_id(), bbp_get_private_status_id(), bbp_get_hidden_status_id() ) ) ) {
 ...
 * @return int
 */
function bbp_get_forums_per_page( $default = 15 ) {

	// Get database option and cast as integer
	$retval = get_option( '_bbp_forums_per_page', $default );
