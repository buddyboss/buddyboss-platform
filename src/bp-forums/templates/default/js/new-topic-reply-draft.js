	...
	class TopicReplyDraft {
		...
		start() {
			...
			if (
				...
				0 === parseInt( BP_Nouveau.forums.params.bb_current_user_id )
			) {
				return;
			}
		...
		addListeners() {
			...
			if (this.is_bb_theme) {
				$( document ).on(
					...
					this.setupOnOpenTopicReplyModal.bind( this )
				);
				$( document ).on(
				...
				$( document ).on(
					...
					this.clearOnCloseTopicReplyModal.bind( this )
				);
			} else {
			...
			$( document ).on(
				...
				this.submitTopicReplyDraftForm.bind( this )
			);
			...
			this.currentForm.on(
				...
				this.discardTopicReplyDraftForm.bind( this )
			);
		}
		...
		setupTopicReplyDraftKeys(event) {
			...
			if (this.currentForm.find( "#bbp_forum_id" ).length > 0) {
				...
				if (this.bbp_topic_id > 0 && this.bbp_reply_to === 0) {
		...
		resetTopicReplyDraftPostForm() {
			...
			var editor_key                  = target.find( ".bbp-the-content" ).data( "key" ),
				...
				document_dropzone_container = target.find(
					"#forums-post-document-uploader"
				),
				video_dropzone_container    = target.find( "#forums-post-video-uploader" ),
				gif_attached_container      = target.find(
					"#whats-new-attachments .forums-attached-gif-container"
				);
			...
			if (
				"undefined" !== typeof media_dropzone_container.length &&
				0 < media_dropzone_container.length
			) {
			...
			if (
				"undefined" !== typeof document_dropzone_container.length &&
				0 < document_dropzone_container.length
			) {
				var document_dropzone_obj_key = document_dropzone_container.data( "key" );
				bp.Nouveau.Media.resetForumsDocumentComponent(
					document_dropzone_obj_key
				);
			...
			if (
				"undefined" !== typeof video_dropzone_container.length &&
				0 < video_dropzone_container.length
			) {
			...
			if (
				...
				target
					.find( "#whats-new-toolbar #forums-gif-button" )
					.removeClass( "active" );
		...
		collectTopicReplyDraftActivity() {
			...
			if (
				"undefined" !== typeof meta.bbp_media &&
				"" !== meta.bbp_media &&
				"[]" !== meta.bbp_media
			) {
				media_valid = true;
			...
			if (
				"undefined" !== typeof meta.bbp_document &&
				"" !== meta.bbp_document &&
				"[]" !== meta.bbp_document
			) {
				media_valid = true;
			...
			if (
				"undefined" !== typeof meta.bbp_video &&
				"" !== meta.bbp_video &&
				"[]" !== meta.bbp_video
			) {
				media_valid = true;
			...
			if (
				"undefined" !== typeof meta.bbp_media_gif &&
				"" !== meta.bbp_media_gif &&
				"[]" !== meta.bbp_media_gif
			) {
				media_valid = true;
			...
			if (
				"undefined" !== typeof meta.link_preview_data &&
				"" !== meta.link_preview_data &&
				"[]" !== meta.link_preview_data
			) {
				media_valid = true;
			...
			if (
				...
				"" === $( $.parseHTML( meta.bbp_topic_content ) ).text().trim() &&
				! media_valid
			) {
				content_valid = false;
			} else if (
				"reply" === this.topic_reply_draft.object &&
				"undefined" !== typeof meta.bbp_reply_content &&
				"" === $( $.parseHTML( meta.bbp_reply_content ) ).text().trim() &&
				! media_valid
			) {
				content_valid = false;
			}
		...
		checkedTopicReplyDataChanged(old_data, new_data) {
			...
			_.each(
				...
				function (pair) {
					if (
					"undefined" !== typeof old_data[pair] &&
					"undefined" === typeof new_data[pair]
					) {
					...
					"undefined" !== typeof new_data[pair]
						...
						if (
						-1 !== _.indexOf( ["bbp_topic_content", "bbp_reply_content"], pair )
						) {
							if (
							$( $.parseHTML( old_data[pair] ) ).text().trim() !==
							$( $.parseHTML( new_data[pair] ) ).text().trim()
							) {
								self.draft_content_changed = true;
							}
		...
		postTopicReplyDraft(is_force_saved, is_reload_window, is_send_all_data) {
			...
			if ( ! is_reload_window) {
				...
				var draft_data = {
					_wpnonce_post_topic_reply_draft:
				...
				// If the current screen is not the edit screen, then send the request.
		...
		appendTopicDraftData() {
			...
			$form
				.find(
					'#bbp_stick_topic_select option[value="' +
						activity_data.bbp_stick_topic +
						'"]'
				)
				.prop( "selected", true );
			...
			// Tags.
			if (
				"undefined" !== typeof activity_data.bbp_topic_tags &&
				"" !== activity_data.bbp_topic_tags
			) {
		...
		previewDraftMedia($form, activity_data) {
			var self                        = bp.Nouveau.Media,
				...
				gif_container               = $form.find(
					"#whats-new-attachments .forums-attached-gif-container"
				);

			// Media.
			if (
				"undefined" !== typeof dropzone_media_container.length &&
				0 < dropzone_media_container.length &&
				...
				if (draft_medias.length) {
					...
					for (var i = 0; i < draft_medias.length; i++) {
						...
						self.dropzone_obj[m_dropzone_obj_key].emit(
							"addedfile",
							m_mock_file
						);
			...
			// Document.
			if (
				"undefined" !== typeof dropzone_document_container.length &&
				0 < dropzone_document_container.length &&
				...
				if (draft_documents.length) {
					...
					for (var d = 0; d < draft_documents.length; d++) {
						...
						self.dropzone_obj[d_dropzone_obj_key].emit(
							"addedfile",
							d_mock_file
						);
						self.dropzone_obj[d_dropzone_obj_key].emit(
			...
			// Video.
			if (
				"undefined" !== typeof dropzone_video_container.length &&
				0 < dropzone_video_container.length &&
				...
				if (draft_videos.length) {
					...
					for (var v = 0; v < draft_videos.length; v++) {
						...
						self.dropzone_obj[v_dropzone_obj_key].emit(
							"addedfile",
							v_mock_file
						);
						self.dropzone_obj[v_dropzone_obj_key].emit(
			...
			// GIF.
			if (
				"undefined" !== typeof gif_container.length &&
				0 < gif_container.length &&
				"undefined" !== typeof activity_data.bbp_media_gif &&
				"" !== activity_data.bbp_media_gif
			) {
