class TopicReplyDraft {
    ...
    start( currentForm ) {
        ...
        this.setupGlobals( currentForm );
        this.addListeners();
    }

    setupGlobals( currentForm ) {
        // Draft variables.
        ...
        var newPostEvent = new Event( currentForm );
        this.setupTopicReplyDraftKeys(newPostEvent, currentForm);
    ...
    setupTopicReplyDraftKeys(event, currentForm ) {

        // console.error( event );
        // console.error( currentForm );

        console.error( currentForm.find('#bbp_topic_id').val() );

        if (currentForm.find('#bbp_forum_id').length > 0) {
            this.bbp_forum_id = parseInt( currentForm.find('#bbp_forum_id').val() );
        ...
        } else if ( currentForm.find('#bbp_topic_id').length > 0 ) {
            this.bbp_topic_id = parseInt( currentForm.find('#bbp_topic_id').val() );
            this.bbp_reply_to = parseInt(currentForm.find('#bbp_reply_to').val());
    ...
    setupTopicReplyDraftIntervals() {
        if (this.is_bb_theme && $('.bb-modal-box').hasClass('bb-modal-open')) {
            if (!window.topic_reply_local_interval) {
                window.topic_reply_local_interval = setInterval(function () {
                    this.collectTopicReplyDraftActivity();
                }.bind(this), 3000);
            }

            if (!window.topic_reply_ajax_interval) {
                window.topic_reply_ajax_interval = setInterval(function () {
...
(function (exports, $) {
    ...
    forms.each(function (index) {
        ...
        bp.Nouveau.TopicReplyDraft.start( currentForm );
