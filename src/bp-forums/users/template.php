...
function bbp_get_user_subscribe_link( $args = '', $user_id = 0, $wrap = true ) {
	...
	if ( empty( $topic_id ) && ! empty( $forum_id ) ) {
		...
		$html = sprintf( '<span class="bb-topic-content-meta-sep">%s</span><span id="subscribe-%d"  %s><a href="%s" class="subscription-toggle" data-forum="%d">%s</a></span>%s', $r['before'], $forum_id, $sub, $url, $forum_id, $text, $r['after'] );
		...
		$sub  = $is_subscribed ? ' class="is-subscribed"' : '';
		$html = sprintf( '<span class="bb-topic-content-meta-sep">%s</span><span id="subscribe-%d"  %s><a href="%s" class="subscription-toggle" data-topic="%d">%s</a></span>%s', $r['before'], $topic_id, $sub, $url, $topic_id, $text, $r['after'] );
