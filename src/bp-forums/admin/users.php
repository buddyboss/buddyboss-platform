...
class BBP_Users_Admin {
	...
	public static function secondary_role_display( $profileuser ) {
		...
		<h3><?php esc_html_e( 'Forums', 'buddyboss' ); ?></h3>

		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="bbp-forums-role"><?php esc_html_e( 'Forum Role', 'buddyboss' ); ?></label></th>
					<td>
						...
						<select name="bbp-forums-role" id="bbp-forums-role">
							...
							<?php else : ?>
								...
								<option value="" selected="selected"><?php esc_html_e( '&mdash; No role for these forums &mdash;', 'buddyboss' ); ?></option>
	...
	public static function user_role_bulk_dropdown() {
		...
		<label class="screen-reader-text" for="bbp-new-role"><?php esc_html_e( 'Change forum role to&hellip;', 'buddyboss' ) ?></label>
		<select name="bbp-new-role" id="bbp-new-role" style="display:inline-block; float:none;">
			<option value=''><?php esc_html_e( 'Change forum role to&hellip;', 'buddyboss' ) ?></option>
			<?php foreach ( $dynamic_roles as $role => $details ) : ?>
				<option value="<?php echo esc_attr( $role ); ?>"><?php echo bbp_translate_user_role( $details['name'] ); ?></option>
			<?php endforeach; ?>
		</select><?php submit_button( __( 'Change', 'buddyboss' ), 'secondary', 'bbp-change-role', false );
	...
	public function user_role_bulk_change() {
		...
		// Bail if this isn't a Forums action
	...
	public static function user_role_column( $columns = array() ) {
		$columns['role']          = __( 'Site Role',  'buddyboss' );
		$columns['bbp_user_role'] = __( 'Forum Role', 'buddyboss' );
	 ...
	 * @param int $user_id
	 *
	 * @return string Displayable Forums user role
	 */
	public static function user_role_row( $retval = '', $column_name = '', $user_id = 0 ) {

		// Only looking for Forums' user role column
