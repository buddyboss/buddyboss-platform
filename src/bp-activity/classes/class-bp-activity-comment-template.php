...
class BP_Activity_Comment_Template {
	...
	public function __construct( $args ) {
		...
		if ( ! empty( $include ) ) {
			$this->activities = bp_activity_get_specific_comment(
				array(
			...
			$this->activities = bp_activity_comment_get(
				array(
					'display_comments'  => $display_comments,
					'max'               => $max,
					'count_total'       => $count_total,
		...
		if ( ! empty( $parent_ids ) ) {
			$activity_parents = bp_activity_get_specific_comment( array( 'activity_ids' => $parent_ids ) );
		}
