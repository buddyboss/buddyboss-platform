...
class BP_Akismet {
	...
	function comment_row_action( $actions, $activity ) {
		...
		if ( $desc ) {
			echo '<span class="akismet-status"><a href="' . esc_url( bp_get_admin_url( 'admin.php?page=bp-activity&amp;action=edit&aid=' . $activity['id'] ) ) . '#bp_activity_history">' . htmlspecialchars( $desc, ENT_QUOTES ) . '</a></span>';
		}
	...
	public function add_activity_spam_button() {
		...
		// By default, only handle activity updates and activity comments.
		if ( ! in_array( bp_get_activity_type(), self::get_activity_types() ) ) {
			return;
		}
	...
	public function add_activity_comment_spam_button() {
		...
		if ( empty( $current_comment ) || ! in_array( $current_comment->type, self::get_activity_types() ) ) {
			return;
		}
