...
// Repair repeater field values for users.
add_filter( 'bp_repair_list', 'bb_xprofile_repeater_field_value_repair' );
...
/**
 * Add xprofile repeater field values repair list item.
 *
 ...
 * @return array Repair list items.
 */
function bb_xprofile_repeater_field_value_repair( $repair_list ) {
	$repair_list[] = array(
		'bp-xprofile-repeater-field-value-repair',
		esc_html__( 'Repair BuddyBoss profile repeater field values', 'buddyboss' ),
		'bb_xprofile_repeater_field_value_repair_callback',
	);
	return $repair_list;
}

/**
 * This function will work as migration process which will add/update repeater field values to main template field.
 *
 * @since BuddyBoss [BBVERSION]
 */
function bb_xprofile_repeater_field_value_repair_callback() {
	static $repeater_groups_ids = array();
	global $wpdb;
	...
	if ( isset( $repeater_groups_ids[ $cache_key ] ) ) {
		...
		$repeater_groups = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT DISTINCT object_id FROM {$bp->profile->table_name_meta} WHERE `object_type` = %s AND `meta_key` = %s AND `meta_value` = %s",
				'group',
				'is_repeater_enabled',
				'on'
			)
		...
		$repeater_groups                   = wp_parse_id_list( $repeater_groups );
		$repeater_groups_ids[ $cache_key ] = $repeater_groups;
	}

	if ( ! empty( $repeater_groups ) ) {

		$offset = isset( $_POST['offset'] ) ? (int) sanitize_text_field( wp_unslash( $_POST['offset'] ) ) : 0;
		$users  = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->users} LIMIT 50 OFFSET %d", $offset ) );
		...
		if ( ! empty( $users ) ) {
			foreach ( $users as $user ) {
				...
				// Repeater group loop.
				foreach ( $repeater_groups as $group_id ) {
					$repeater_main_fields = bp_get_repeater_template_field_ids( $group_id );

					// Repeater group field loop.
					if ( ! empty( $repeater_main_fields ) ) {
						foreach ( $repeater_main_fields as $repeater_main_field_id ) {

							$sub_field_values = $wpdb->get_col(
								$wpdb->prepare(
									"SELECT value FROM {$bp->profile->table_name_fields} AS f LEFT JOIN {$bp->profile->table_name_data} AS fd ON f.id = fd.field_id LEFT JOIN {$bp->profile->table_name_meta} AS fm ON f.id = fm.object_id WHERE f.group_id = %d AND f.id != %d AND fd.user_id = %d AND fm.meta_key = '_cloned_from' AND fm.meta_value = %d",
									$group_id,
									$repeater_main_field_id,
									$user,
									$repeater_main_field_id
								)
							);
							...
							if ( ! empty( $sub_field_values ) ) {
								$sub_field_value = implode( ' ', $sub_field_values );
							}

							$is_exists = $wpdb->get_col(
								$wpdb->prepare(
							...
							if ( ! empty( $is_exists ) ) {
								$is_exists = current( $is_exists );

								$wpdb->query(
									$wpdb->prepare(
										"UPDATE {$bp->profile->table_name_data} SET value = %s, last_updated = %s WHERE id = %d",
										$sub_field_value,
								...
								$wpdb->query(
									$wpdb->prepare(
										"INSERT INTO {$bp->profile->table_name_data} ( `field_id`, `user_id`, `value`, `last_updated` )  VALUES ( %d, %d, %s, %s )",
										$repeater_main_field_id,
										$user,
										$sub_field_value,
										bp_core_current_time()
			...
			return array(
				'status'  => 1,
				'message' => __( 'Repairing BuddyBoss profile repeater field values &hellip; Complete!', 'buddyboss' ),
			);
		}
	} else {
		return array(
