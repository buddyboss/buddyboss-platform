	...
	if ( isset( $_POST['field_ids'] ) ) {
		...
		if ( ! empty( $errors ) && $validations ) {
			...
			foreach ( (array) $posted_field_ids as $field_id ) {
				...
				// We need to pass post value, not xprofile_get_field_data's value.
				// If we get value from xprofile_get_field_data this function then date format change and it will not validate as per Y-m-d 00:00:00 format.
