...
//		}
	...
	public function generate_previews( $attachment_id, $document_id ) {
		...
		if ( empty( $is_preview_generated ) ) {
			...
			if ( 'pdf' === $extension ) {
				...
				self::generate_pdf_previews( array( $attachment_id ), true, $document_id );
				remove_filter( 'wp_image_editors', array( $this, 'bp_document_wp_image_editors' ) );
			} elseif ( in_array( $extension, bp_get_document_preview_code_extensions(), true ) ) {
				self::generate_code_previews( $attachment_id );
			}
		}
	}

	static function generate_code_previews( $attachment_id ) {
		$extension             = bp_document_extension( $attachment_id );
	 ...
	 */
	static function generate_pdf_previews( $ids, $check_mime_type = false, $document_id ) {
