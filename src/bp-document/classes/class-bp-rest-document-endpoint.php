...
class BP_REST_Document_Endpoint extends WP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( true === $retval && ! empty( $request['group_id'] ) && bp_is_active( 'groups' ) ) {
			...
			} else if ( ! bp_current_user_can( 'bp_moderate' ) && ! empty( $group->id ) && 'public' !== bp_get_group_status( $group ) && isset( $user_groups['groups'] ) && ! in_array( $group->id, $user_groups['groups'], true ) ) {
				$retval = new WP_Error(
					'bp_rest_authorization_required',
