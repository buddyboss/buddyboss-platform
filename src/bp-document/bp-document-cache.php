...
function bp_document_clear_cache_for_document( $document ) {
	...
	wp_cache_delete( 'bb_document_activity_' . $document->id, 'bp_video' ); // Used in bb_moderation_get_media_record_by_id().
}
...
function bp_document_clear_cache_for_deleted_document( $deleted_ids ) {
	foreach ( (array) $deleted_ids as $deleted_id ) {
		...
		wp_cache_delete( 'bb_document_activity_' . $deleted_id, 'bp_video' ); // Used in bb_moderation_get_media_record_by_id().
	}
}
...
function bp_document_clear_document_user_object_cache_on_delete( $documents ) {
	if ( ! empty( $documents[0] ) ) {
		foreach ( (array) $documents[0] as $deleted_document ) {
			...
			wp_cache_delete( 'bb_document_activity_' . $deleted_document->id, 'bp_video' ); // Used in bb_moderation_get_media_record_by_id().
...
function bp_document_clear_cache_for_folder( $folder ) {
	...
	wp_cache_delete( 'get_folder_document_ids_' . $folder->id, 'bp_document_folder' );
	wp_cache_delete( 'get_folder_attachment_ids_' . $folder->id, 'bp_document_folder' );
	wp_cache_delete( 'get_folder_children_' . $folder->id, 'bp_document_folder' );
}
...
function bp_document_clear_cache_for_deleted_folder( $deleted_ids ) {
	foreach ( (array) $deleted_ids as $deleted_id ) {
		...
		wp_cache_delete( 'get_folder_document_ids_' . $deleted_id, 'bp_document_folder' );
		wp_cache_delete( 'get_folder_attachment_ids_' . $deleted_id, 'bp_document_folder' );
		wp_cache_delete( 'get_folder_children_' . $deleted_id, 'bp_document_folder' );
	}
}
