...
class BP_Moderation_Document extends BP_Moderation_Abstract {
	...
	public function __construct() {
		 ...
		 */
		add_filter( 'bp_suspend_document_get_where_conditions', array( $this, 'update_where_sql' ), 10, 3 );
	...
	public function update_where_sql( $where, $suspend, $args ) {
		...
		if ( ! empty( $blocked_by_users_ids ) ) {
			...
			$where['moderation_where'] .= "( d.user_id NOT IN ( " . implode( ', ', $blocked_by_users_ids ) . " ) OR d.privacy IN ( 'grouponly' ) )";
		}
