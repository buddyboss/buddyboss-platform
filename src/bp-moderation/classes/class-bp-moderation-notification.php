...
class BP_Moderation_Notification extends BP_Moderation_Abstract {
	...
	public function update_where_sql( $sql_where, $tbl_alias, $args = array() ) {
		...
		if ( bp_is_moderation_member_blocking_enable( 0 ) ) {
			...
			$moderation_query = $wpdb->prepare( "SELECT item_id FROM {$bp->table_prefix}bp_suspend s INNER JOIN {$bp->table_prefix}bp_moderation mr ON mr.moderation_id = s.id WHERE mr.user_id = %d AND ( hide_parent = %d OR hide_sitewide = %d OR reported = %d ) AND item_type = %s", bp_loggedin_user_id(), 1, 1, 1, 'user' );
