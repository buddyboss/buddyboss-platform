...
class BP_Suspend_Document extends BP_Suspend_Abstract {
	...
	protected function get_related_contents( $document_id, $args = array() ) {
		...
		if ( bp_is_active( 'activity' ) && ! empty( $document ) && ! empty( $document->id ) && ! empty( $document->activity_id ) ) {
			...
			if (
				(
					'hide' === $args['action'] &&
					true === bp_moderation_is_activity_related_content_hidden( $document )
				) ||
				(
					'unhide' === $args['action'] &&
					false === bp_moderation_is_activity_related_content_hidden( $document )
				)
			) {
	...
	public function is_activity_document_hidden( $hidden, $activity_data ) {
		...
		if ( ! empty( $unhidden_document_ids ) ) {
			return bp_moderation_is_content_hidden( $unhidden_document_ids, BP_Moderation_Document::$moderation_type );
		} else {
			return $hidden;
		}
	}
