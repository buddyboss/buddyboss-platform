...
class BP_Suspend_Document extends BP_Suspend_Abstract {
	...
	public function manage_hidden_document( $document_id, $hide_sitewide, $args = array() ) {
		...
		$args['parent_id'] = ! empty( $args['parent_id'] ) ? $args['parent_id'] : $this->item_type . '_' . $document_id;
		...
		if ( $this->background_disabled ) {
			...
			if ( as_has_scheduled_action( 'bb_as_hide_related_content', array( $document_id, $hide_sitewide, $args ), $group_name ) ) {
				return;
			}
			$action_id = as_enqueue_async_action( 'bb_as_hide_related_content', array( $document_id, $hide_sitewide, $args ), $group_name, true );
			if ( $action_id ) {
				bb_insert_as_meta( $action_id, $this->item_type, $group_name, $document_id, $args['parent_id'] );
			} else {
				error_log( 'duplicate' );
			}
		}
	...
	public function manage_unhidden_document( $document_id, $hide_sitewide, $force_all, $args = array() ) {
		...
		if ( $this->background_disabled ) {
			...
			if ( as_has_scheduled_action( 'bb_as_unhide_related_content', array( $document_id, $hide_sitewide, $force_all, $args ), $group_name ) ) {
				return;
			}
			$action_id = as_enqueue_async_action( 'bb_as_unhide_related_content', array( $document_id, $hide_sitewide, $force_all, $args ), $group_name, true );
			if ( $action_id ) {
				bb_insert_as_meta( $action_id, $this->item_type, $group_name, $document_id, $args['parent_id'] );
			} else {
				error_log( 'duplicate' );
