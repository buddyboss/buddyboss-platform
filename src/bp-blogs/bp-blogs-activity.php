 ...
 *
 * @since BuddyPress 2.5.0 This was moved out of the BP_Blogs_Component class.
 *
 ...
 *
 * @since BuddyPress 2.0.0
 * @since BuddyPress 2.5.0 Allow custom post types to sync their comments with activity ones
 *
 ...
 *
 * @since BuddyPress 2.0.0
 * @since BuddyPress 2.5.0 Add the $delected parameter
 *
 ...
 * @since BuddyPress 2.5.0 Used for any synced Post type comments, in wp-admin or front-end contexts.
 *
 ...
 * @return string Constructed activity action.
 */
function bp_blogs_format_activity_action_new_custom_post_type_feed( $action, $activity ) {
	$blog_url  = bp_blogs_get_blogmeta( $activity->item_id, 'url' );
	$blog_name = bp_blogs_get_blogmeta( $activity->item_id, 'name' );
	...
	/**
	 * When the custom post type is published we are faking an activity object
	 * to which we add 2 properties :
	 * - the post url
	 * - the post title
	 * This is done to build the 'post link' part of the activity
	...
	// Legacy filter - requires the post object.
	if ( has_filter( 'bp_blogs_activity_new_custom_post_action' ) ) {
		switch_to_blog( $activity->item_id );
		$post = get_post( $activity->secondary_item_id );
		restore_current_blog();

		if ( ! empty( $post ) && ! is_wp_error( $post ) ) {
			$action = apply_filters( 'bp_blogs_activity_new_custom_post_action', $action, $post, $post_url );
		}
	}
...
function bb_filter_activity_get_filter_sql( $filter_sql, $filter_array ) {
	...
	if ( ! empty( $filter_array['action'] ) ) {
		...
		if ( in_array( 'new_blog_comment', $action_array, true ) && ! bp_disable_blogforum_comments() && count( $action_array ) > 1 ) {
			...
			if ( ! empty( $post_type ) && ! bp_activity_post_type_get_tracking_arg( 'new_blog_comment', 'comment_action_id' ) ) {
				$activity_ids = $wpdb->get_col( $wpdb->prepare( "SELECT activity_id FROM {$bp->activity->table_name_meta} WHERE meta_key = %s", "bp_blogs_{$post_type}_comment_id" ) ); // phpcs:ignore WordPress.DB

				if ( ! empty( $activity_ids ) ) {
					$filter_blog_comment = 'a.id IN (' . implode( ',', $activity_ids ) . ')';
					if ( ! empty( $filter_blog_comment ) ) {
