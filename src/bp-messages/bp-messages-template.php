			...
			if ( ! isset( $chunks[ $i ] ) ) {
				...
				switch ( $seconds ) {
					case YEAR_IN_SECONDS:
						$output = $count < 2 ? bp_core_get_format_date( $old_last_date, 'l, F j' ) : bp_core_get_format_date( $old_last_date, 'M j, Y' );
						break;
					case WEEK_IN_SECONDS:
						...
						if ( $start_week <= $last_message_date && $end_week >= $last_message_date ) {
							...
							$output = bp_core_get_format_date( $old_last_date, 'l, F j' );
						}
						break;
					case DAY_IN_SECONDS:
						$start_week = bb_get_week_start_timestamp();
