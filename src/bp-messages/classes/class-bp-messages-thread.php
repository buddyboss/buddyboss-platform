...
class BP_Messages_Thread {
	...
	public static function delete( $thread_id = 0, $user_id = 0 ) {
		...
		$message_col = BP_Messages_Message::get( array(
			'selector'  => 'column',
			'fields'    => 'ids',
			'thread_id' => $thread_id,
		) );

		$message_ids = $message_col['results'];
		...
		$update_messages = BP_Messages_Message::get( array(
			'selector'  => 'column',
			'fields'    => 'ids',
			'thread_id' => $thread_id,
			'sender_id' => $user_id,
		...
		$update_message_ids = $update_messages['results'];
	...
	public static function user_is_sender( $thread_id ) {
		...
		$senders = BP_Messages_Message::get( array(
			'selector'  => 'column',
			'fields'    => 'sender_id',
			'thread_id' => $thread_id,
		) );

		$sender_ids = $senders['results'];
