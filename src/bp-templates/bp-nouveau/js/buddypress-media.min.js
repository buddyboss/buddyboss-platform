					...
					success: function ( response ) {
						if ( response.success ) {
							...
							if ( response.data && response.data.source_album_id && response.data.source_album_id > 0 ) {
								...
								var currentAlbumId = parseInt( BP_Nouveau.media.current_album );
							...
							if ( response.data && response.data.album_id && response.data.album_id > 0 ) {
								var currentAlbumId = parseInt( BP_Nouveau.media.current_album );
								if ( currentAlbumId === response.data.album_id ) {
									var $albumHeader = $( '.bb-single-album-header' );
									if ( $albumHeader.length > 0 ) {
										// Update photo count
