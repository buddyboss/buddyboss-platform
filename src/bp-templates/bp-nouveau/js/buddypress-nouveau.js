...
( function ( exports, $ ) {
	...
	bp.Nouveau = {
		...
		objectRequest: function ( data ) {
			...
			// if object is members, media, document, groups and object nav does not exists fallback to scope = all.
			if ( [ 'members', 'media', 'activity', 'document', 'groups' ].includes( data.object ) && ! $( this.objectNavParent + ' [data-bp-scope="' + data.scope + '"]' ).length ) {
				data.scope = 'all';
