...
( function ( exports, $ ) {
	...
	bp.Nouveau = {
		...
		objectRequest: function ( data ) {
			...
			// if object is members, media, document, video and object nav does not exists fallback to scope = all.
			if ( [ 'members', 'media', 'activity', 'document', 'video' ].includes( data.object ) && ! $( this.objectNavParent + ' [data-bp-scope="' + data.scope + '"]' ).length ) {
				data.scope = 'all';
