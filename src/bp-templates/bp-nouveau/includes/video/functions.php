...
function bp_nouveau_video_localize_scripts( $params = array() ) {
	...
	$params['video'] = array(
		...
		'dropzone_video_message'             => __( '<strong>Add Videos</strong> Or drag and drop', 'buddyboss' ),
...
function bp_nouveau_video_activity_edit_button( $buttons, $activity_id ) {
	if (
		...
		(
			'video_get_activity' === $_REQUEST['action'] ||
			'video_get_video_description' === $_REQUEST['action']
		)
	) { // phpcs:ignore
