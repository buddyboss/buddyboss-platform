...
function bp_document_get_child_folders( $folder_id = 0, $parent_folder = '' ) {
	...
	$documents_folder_query = $wpdb->prepare( "select * from ( select * from {$document_folder_table} order by parent, id) folder_sorted, (select @pv := %d) initialisation where find_in_set(parent, @pv) and length(@pv := concat(@pv, ',', id))", $folder_id );
