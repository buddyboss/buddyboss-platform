...
function bp_document_download_file( $attachment_id, $type = 'document' ) {
	...
	if ( 'document' === $type ) {
		...
		if ( ! is_dir( $upload_dir ) ) {
			...
			if ( version_compare( $phpVersion, '7.1', '>=' ) && class_exists( 'BuddyBossPlatform\ZipStream\ZipStream' ) ) {
				$options = new \BuddyBossPlatform\ZipStream\Option\Archive();
				...
				$zip = new BuddyBossPlatform\ZipStream\ZipStream( $file_name, $options );
