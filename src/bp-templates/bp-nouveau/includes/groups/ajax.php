...
function bb_nouveau_ajax_group_subscription() {
	$response = array(
		'feedback'              => esc_html__( 'There was a problem subscribing/unsubscribing.', 'buddyboss-platform' ),
	...
	if ( ! bb_is_enabled_subscription( 'group' ) ) {
		wp_send_json_error(
			array(
				'feedback'              => esc_html__( 'Subscriptions are no longer active.', 'buddyboss-platform' ),
				'is_group_subscription' => true,
			)
		);
	}
	...
	$group_name = bp_create_excerpt( $group_name, 35, array( 'ending' => __( '&hellip;', 'buddyboss-platform' ) ) );
	$group_name = sprintf(
		'<strong>%s</strong>',
		$group_name
	);
	...
	switch ( $_POST['action'] ) {
		...
		case 'groups_subscribe':
			if ( $is_subscription ) {
				$response = array(
					'feedback'              => esc_html__( 'You are already subscribe this group.', 'buddyboss-platform' ),
					'type'                  => 'error',
					'is_group_subscription' => true,
				);
			} else {
				...
				if ( is_wp_error( $subscription_id ) ) {
					$response = array(
						'feedback'              => sprintf(
							...
							esc_html__( 'There was a problem subscribing to %s.', 'buddyboss-platform' ),
							$group_name
						),
						'type'                  => 'error',
						'is_group_subscription' => true,
					...
					$response = array(
						...
						'feedback'              => sprintf(
						/* translators: Group name. */
							esc_html__( 'You\'ve been subscribed to %s.', 'buddyboss-platform' ),
							$group_name
						),
					);
				}
		...
		case 'groups_unsubscribe':
			if ( $is_subscription && bb_delete_subscription( $is_subscription ) ) {
				...
				$response = array(
					...
					'feedback'              => sprintf(
					/* translators: Group name. */
						esc_html__( 'You\'ve been unsubscribed from %s.', 'buddyboss-platform' ),
						$group_name
					),
				);
			} else {
				$response = array(
					'feedback'              => sprintf(
					/* translators: Group name. */
						esc_html__( 'There was a problem unsubscribing from %s.', 'buddyboss-platform' ),
						$group_name
					),
					'type'                  => 'error',
					'is_group_subscription' => true,
