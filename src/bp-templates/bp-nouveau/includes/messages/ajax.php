	...
	while ( bp_thread_messages() ) :
		...
		if ( $group_id && $message_from && 'group' === $message_from ) {
			...
			if ( $message_left && 'yes' === $message_left ) {
				/* translators: %s: Group Name */
				$content    = '<p class="joined">' . sprintf( __( 'Left %s', 'buddyboss' ), '<strong>' . ucwords( $group_name ) . '</strong>' ) . '</p>';
				...
				if ( is_array( $left_users ) ) {
					if ( 2 === count( $left_users ) ) {
						...
						if ( 5 > count( $left_users ) ) {
							...
							$content = '<p class="joined">' . sprintf( __( 'Left <strong>%1$s</strong>, along with <strong class="bp-tooltip" data-bp-tooltip-pos="up" data-bp-tooltip="%2$s">%3$d others</strong>', 'buddyboss' ), ucwords( $group_name ), implode( ', ', $left_members ), count( $left_users ) ) . '</p>';
						}
					}
				}
				...
				if ( is_array( $joined_users ) ) {
					if ( 2 === count( $joined_users ) ) {
						...
						if ( 5 > count( $joined_users ) ) {
							...
							/* translators: 1. Group Name. 2. Member Names. 3. Member Count. */
							$content = '<p class="joined">' . sprintf( __( 'Joined <strong>%1$s</strong>, along with <strong class="bp-tooltip" data-bp-tooltip-pos="up" data-bp-tooltip="%2$s">%3$d others</strong>', 'buddyboss' ), ucwords( $group_name ), implode( ', ', $joined_members ), count( $joined_users ) ) . '</p>';
						}
					}
				}
		...
		if ( ! $has_message_updated && bp_is_active( 'media' ) && ( ( ( empty( $is_group_thread ) || ( ! empty( $is_group_thread ) && ! bp_is_active( 'groups' ) ) ) && bp_is_messages_media_support_enabled() ) || ( bp_is_active( 'groups' ) && ! empty( $is_group_thread ) && bp_is_group_media_support_enabled() ) ) ) {
		...
		if ( ! $has_message_updated &&  bp_is_active( 'video' ) && bp_is_messages_video_support_enabled() ) {
		...
		if ( ! $has_message_updated &&  bp_is_active( 'media' ) && ( ( ( empty( $is_group_thread ) || ( ! empty( $is_group_thread ) && ! bp_is_active( 'groups' ) ) ) && bp_is_messages_document_support_enabled() ) || ( bp_is_active( 'groups' ) && ! empty( $is_group_thread ) && bp_is_group_document_support_enabled() ) ) ) {
		...
		if ( ! $has_message_updated &&  bp_is_active( 'media' ) && ( ( ( empty( $is_group_thread ) || ( ! empty( $is_group_thread ) && ! bp_is_active( 'groups' ) ) ) && bp_is_messages_gif_support_enabled() ) || ( bp_is_active( 'groups' ) && ! empty( $is_group_thread ) && bp_is_groups_gif_support_enabled() ) ) ) {
