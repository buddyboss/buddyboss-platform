...
function bp_nouveau_ajax_messages_send_reply() {
	...
	if ( bp_is_active( 'media' ) && bp_is_messages_media_support_enabled() ) {
		...
		if ( ! empty( $media_ids ) && bp_has_media( array( 'include' => $media_ids, 'privacy' => array( 'message' ),  'order_by' => 'menu_order', 'sort' => 'ASC' ) ) ) {
			$reply['media'] = array();
			while ( bp_media() ) {
				bp_the_media();
 ...
 */
function bp_nouveau_ajax_get_thread_messages() {
	global $thread_template, $media_template, $document_template, $wpdb;
