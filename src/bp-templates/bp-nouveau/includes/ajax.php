...
function bp_nouveau_object_template_results_members_tabs( $results, $object ) {
	...
	if ( bp_is_active( 'activity' ) && bp_is_activity_follow_active() ) {
		...
		if ( ! empty( $counts['followers'] ) ) {
			add_filter( 'bp_ajax_querystring', 'bp_nouveau_object_template_results_members_followers_scope', 20 );
			bp_has_members( bp_ajax_querystring( 'members' ) );
			$results['scopes']['followers'] = $GLOBALS["members_template"]->total_member_count;
			remove_filter( 'bp_ajax_querystring', 'bp_nouveau_object_template_results_members_followers_scope', 20 );
		}
	}
...
function bp_nouveau_object_template_results_members_followers_scope( $querystring ) {
	...
	$follower_comma_separated_string = bp_get_follower_ids( $args );
	$querystring['include']           = $follower_comma_separated_string;
