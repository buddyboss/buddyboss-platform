...
$size         = base64_decode( $_REQUEST['size'] );
...
if ( isset( $explode_arr ) && ! empty( $explode_arr ) && isset( $explode_arr[1] ) && (int) $explode_arr[1] > 0 &&
	...
	$media_id      = (int) $explode_arr[1];
	$id1           = (int) $explode_arr1[1];
	$media_privacy = ( function_exists( 'bp_media_user_can_manage_media' ) ) ? bp_media_user_can_manage_media( $id1, bp_loggedin_user_id() ) : true;
	$can_view      = ( true === (bool) $media_privacy['can_view'] ) ? true : false;

	if ( $can_view && wp_attachment_is_image( $media_id ) ) {

		$mime_type       = get_post_mime_type( $media_id );
		$output_file_src = bp_media_scaled_image_path( $media_id, $size );
