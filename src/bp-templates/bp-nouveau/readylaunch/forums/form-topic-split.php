			...
			<form id="split_topic" class="bb-rl-forum-form" name="split_topic" method="post" action="<?php the_permalink(); ?>">
				...
				<div class="bb-rl-forum-modal-header">
					<h3>
						...
						printf( esc_html__( 'Split discussion "%s"', 'buddyboss-platform' ), bbp_get_topic_title() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- bbp_get_topic_title() is escaped via its own bbp_get_topic_title filter; format string uses esc_html__().
						?>
					</h3>
				</div>
							...
							<fieldset class="bbp-form bb-rl-forum-fieldset">
								...
								<div class="bb-rl-forum-fieldset">
									<div class="bp-radio-wrap">
										...
										<label for="bbp_topic_split_option_reply"><?php /* translators: %s: forum title. */ printf( wp_kses_post( __( 'New discussion in <strong>%s</strong> titled:', 'buddyboss-platform' ) ), esc_html( bbp_get_forum_title( bbp_get_topic_forum_id( bbp_get_topic_id() ) ) ) ); ?></label>
									</div>
									<input type="text" id="bbp_topic_split_destination_title" value="<?php /* translators: %s: Topic title */ printf( esc_attr__( 'Split: %s', 'buddyboss-platform' ), wp_kses_post( bbp_get_topic_title() ) ); ?>" tabindex="<?php bbp_tab_index(); ?>" size="35" name="bbp_topic_split_destination_title" />
								</div>
