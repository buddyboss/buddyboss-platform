<?php
/**
 * Readylaunch - Messages single list template.
 *
		...
		<# if ( ! data.is_group_notice ) { #>
			<# if ( ! data.is_user_suspended && ! data.is_user_blocked ) { #>
				<div class="bp-avatar-wrap">
					...
					<# } else if ( data.is_user_suspended || data.is_user_blocked  ) { #>
						...
						<# } #>
					<# } else { #>
						<a href="{{data.sender_link}}" class="bp-user-avatar" aria-label="{{{data.sender_name}}}" data-bb-hp-profile="{{data.sender_id}}">
							<img class="avatar" src="{{{data.sender_avatar}}}" alt="" />
			...
			<div class="bp-single-message-content">
				...
				<div class="message-metadata">
					...
					<# } else { #>
						<a href="{{data.sender_link}}" class="bp-user-link" data-bb-hp-profile="{{data.sender_id}}">
				...
				<# if ( data.video ) { #>
					<div class="bb-activity-video-wrap bb-video-length-{{data.video.length}}">
						<# for ( i in data.video ) { #>
							<div class="bb-rl-activity-video-elem <# if ( -1 !== data.video[i].thumbnail.toLowerCase().indexOf( 'video-placeholder.jpg' ) ) { #>has-no-thumbnail<# } #>">
								<#
								if ( data.video[i].id && 1 === data.video.length ) { #>
									{{{data.video[i].video_html}}}
								<# } else if ( data.video[i].id ) { #>
									<a class="bb-open-video-theatre bb-rl-video-cover-wrap bb-rl-item-cover-wrap hide" data-id="{{data.video[i].id}}" data-attachment-id="{{data.video[i].attachment_id}}" data-attachment-full="{{data.video[i].full}}" data-privacy="{{data.video[i].privacy}}" href="#">
								...
								<# if ( data.document[i].id ) { #>
									<div class="bb-rl-document-description-wrap">
										...
										<a href="{{data.document[i].url}}"
											class="bb-rl-document-detail-wrap bb-rl-open-document-theatre"
