...
( function ( exports, $ ) {
	...
	bp.Nouveau.TopicReplyDraft = function(currentForm) {
		...
		this.addListeners = function() {
			...
			this.currentForm.find( '.bb-rl-forum-modal-close' ).on(
			...
			// connection the member can already have the composer OPEN by the
			// time the handlers above exist. jQuery does not replay an
			// `bbp_after_load_reply_form` that has already fired, so the draft
			// was never offered and the autosave intervals never started for
			// the rest of that modal session: an empty composer over a draft
			// the member does have, and nothing they typed saved either.
			// Running the same setup once, now, is what the missed event would
			// have done - and displayTopicReplyDraft() still refuses to clobber
			// anything typed in the meantime, via hasMemberTypedContent()
			// (PROD-9621 Q12).
		...
		this.updateLeadDiscussionTagsInForm = function() {
			...
			var topic_tags   = [];
			var tagSelector  = '.item-tags ul li';
		...
		this.resetTopicReplyDraftPostForm = function() {
			...
			var editor_key                  = target.find( '.bbp-the-content' ).data( 'key' ),
				$editor,
				$medium_editor,
				media_dropzone_container    = target.find( '#bb-rl-forums-post-media-uploader' ),
				document_dropzone_container = target.find( '#bb-rl-forums-post-document-uploader' ),
				video_dropzone_container    = target.find( '#bb-rl-forums-post-video-uploader' ),
			...
			if ( 'undefined' !== typeof media_dropzone_container.length && 0 < media_dropzone_container.length ) {
				...
				if ( target.find( '#bb-rl-forums-media-button' ) ) {
					target.find( '#bb-rl-forums-media-button' ).parents( '.bb-rl-post-elements-buttons-item' ).removeClass( 'disable no-click' );
				}
			}
			...
			if ( 'undefined' !== typeof document_dropzone_container.length && 0 < document_dropzone_container.length ) {
				...
				if ( target.find( '#bb-rl-forums-document-button' ) ) {
					target.find( '#bb-rl-forums-document-button' ).parents( '.bb-rl-post-elements-buttons-item' ).removeClass( 'disable no-click' );
				}
			}
			...
			if ( 'undefined' !== typeof video_dropzone_container.length && 0 < video_dropzone_container.length ) {
				...
				if ( target.find( '#bb-rl-forums-video-button' ) ) {
					target.find( '#bb-rl-forums-video-button' ).parents( '.bb-rl-post-elements-buttons-item' ).removeClass( 'disable no-click' );
				}
			}

			// Reset GIF.
			if ( 'undefined' !== typeof document_dropzone_container.length && 0 < document_dropzone_container.length ) {
				target.find( '#whats-new-toolbar #bb-rl-forums-gif-button' ).removeClass( 'active' );
				...
				if ( gif_attached_container.length ) {
					...
					target.find( '#bb-rl-forums-gif-button' ).parents( '.bb-rl-post-elements-buttons-item' ).removeClass( 'disable no-click' );
				}
		...
		this.previewDraftMedia = function( $form, activity_data ) {
			var self                        = bp.Nouveau.Media,
				...
				dropzone_document_container = $form.find( '#bb-rl-forums-post-document-uploader' ),
				dropzone_video_container    = $form.find( '#bb-rl-forums-post-video-uploader' ),
			...
			if ( 'undefined' !== typeof dropzone_media_container.length && 0 < dropzone_media_container.length && 'undefined' !== typeof activity_data.bbp_media && '' !== activity_data.bbp_media ) {
				...
				if ( draft_medias.length ) {
					$form.find( 'a#bb-rl-forums-media-button' ).trigger( 'click' );
					...
					for ( var i = 0; i < draft_medias.length; i++ ) {
						...
						bp.Readylaunch.Utilities.createThumbnailFromUrl( m_mock_file, dropzone_media_container );
					...
					bp.Nouveau.Media.addMediaIdsToForumsForm( dropzone_media_container );
					...
					if ( ! _.isNull( self.dropzone_obj[ m_dropzone_obj_key ].files ) && self.dropzone_obj[ m_dropzone_obj_key ].files.length !== 0 ) {
						if ( $form.find( '#bb-rl-forums-document-button' ) ) {
							$form.find( '#bb-rl-forums-document-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'disable' );
						}
						if ( $form.find( '#bb-rl-forums-video-button' ) ) {
							$form.find( '#bb-rl-forums-video-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'disable' );
						}
						if ( $form.find( '#bb-rl-forums-gif-button' ) ) {
							$form.find( '#bb-rl-forums-gif-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'disable' );
						}
						if ( $form.find( '#bb-rl-forums-media-button' ) ) {
							$form.find( '#bb-rl-forums-media-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'no-click' );
						}
					}
			...
			if ( 'undefined' !== typeof dropzone_document_container.length && 0 < dropzone_document_container.length && 'undefined' !== typeof activity_data.bbp_document && '' !== activity_data.bbp_document ) {
				...
				if ( draft_documents.length ) {
					$form.find( 'a#bb-rl-forums-document-button' ).trigger( 'click' );
					...
					bp.Nouveau.Media.addDocumentIdsToForumsForm( dropzone_document_container );
					...
					if ( ! _.isNull( self.dropzone_obj[ d_dropzone_obj_key ].files ) && self.dropzone_obj[ d_dropzone_obj_key ].files.length !== 0 ) {
						if ( $form.find( '#bb-rl-forums-media-button' ) ) {
							$form.find( '#bb-rl-forums-media-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'disable' );
						}
						if ( $form.find( '#bb-rl-forums-video-button' ) ) {
							$form.find( '#bb-rl-forums-video-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'disable' );
						}
						...
						if ( $form.find( '#bb-rl-forums-document-button' ) ) {
							$form.find( '#bb-rl-forums-document-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'no-click' );
						}
					}
			...
			if ( 'undefined' !== typeof dropzone_video_container.length && 0 < dropzone_video_container.length && 'undefined' !== typeof activity_data.bbp_video && '' !== activity_data.bbp_video ) {
				...
				if ( draft_videos.length ) {
					$form.find( 'a#bb-rl-forums-video-button' ).trigger( 'click' );
						...
						var thumbnail_check = setInterval( function() {
							...
							if ( $preview_element.hasClass( 'dz-has-no-thumbnail' ) || $preview_element.hasClass( 'dz-has-thumbnail' ) ) {
								...
								if ( counter.count >= videos.length ) {
									...
									var video_dropzone_container = form.find( '#bb-rl-forums-post-video-uploader' );
					...
					if ( ! _.isNull( self.dropzone_obj[ v_dropzone_obj_key ].files ) && self.dropzone_obj[ v_dropzone_obj_key ].files.length !== 0 ) {
						...
						if ( $form.find( '#bb-rl-forums-video-button' ) ) {
							$form.find( '#bb-rl-forums-video-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'no-click' );
						}
					}
			...
			if ( 'undefined' !== typeof gif_container.length && 0 < gif_container.length && 'undefined' !== typeof activity_data.bbp_media_gif && '' !== activity_data.bbp_media_gif ) {
				...
				if ( 'undefined' !== typeof draft_gif.images ) {
					$form.find( 'a#bb-rl-forums-gif-button' ).trigger( 'click' );
					...
					if ( $( '#bbp_media_gif' ).length ) {
						...
						if ( $form.find( '#bb-rl-forums-gif-button' ) ) {
							$form.find( '#bb-rl-forums-gif-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'no-click' );
						}
						if ( $form.find( '#bb-rl-forums-media-button' ) ) {
							$form.find( '#bb-rl-forums-media-button' ).parents( '.bb-rl-post-elements-buttons-item' ).addClass( 'disable' );
						}
