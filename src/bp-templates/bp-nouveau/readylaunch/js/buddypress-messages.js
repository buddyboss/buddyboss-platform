...
( function( exports, $ ) {
	...
	var bpNouveau                    = BP_Nouveau,
	    ...
	    bbRlArchivedThreads          = bpNouveau.archived_threads,
	    bbRlAnchorPlaceholderText    = bpNouveau.anchorPlaceholderText,
	    bbRlMedia                    = bpNouveau.media,
	    bbRlVideo                    = bpNouveau.video,
	    bbRlDocument                 = bpNouveau.document,
	    bbRlModerationContentNonce   = bpNouveau.nonce.bp_moderation_content_nonce,
	    bbRlNonces                   = bpNouveau.nonces,
	    bbRlMessages                 = bpNouveau.messages,
	    bbRlDeleteConfirmation       = bbRlMessages.delete_confirmation,
	    bbRlDeleteThreadConfirmation = bbRlMessages.delete_thread_confirmation,
	    bbRlMessagesNonces           = bbRlMessages.nonces,
	    bbRlTypeMessage              = bbRlMessages.type_message,
	    bbRlErrors                   = bbRlMessages.errors,
	    bbRlIsBlockedByMembers       = bbRlMessages.is_blocked_by_members,
	    bbRlSingleMedia              = bbRlMessages.single_media,
	    bbRlMultipleMedia            = bbRlMessages.multiple_media,
	    bbRlSingleVideo              = bbRlMessages.single_video,
	    bbRlMultipleVideo            = bbRlMessages.multiple_video,
	    bbRlSingleDocument           = bbRlMessages.single_document,
	    bbRlMultipleDocument         = bbRlMessages.multiple_document,
	    bbRlGifMedia                 = bbRlMessages.gif_media,
	    bbRlVideoDefaultUrl          = bbRlMessages.video_default_url,
	    bbRlToday                    = bbRlMessages.today,
	...
	bp.Nouveau.Messages = {
		...
		checkContentScroll: function() {
			...
			var messagesContentElem = $( '.bb-rl-messages-content' ),
			    messages_list       = messagesContentElem.find( '#bp-message-thread-list' );
			if ( messages_list.prop( 'scrollHeight' ) > messages_list.scrollTop() + messages_list.height() + 22 ) {
				messagesContentElem.find( '.bp-messages-content-wrapper' ).addClass( 'scrolled--up' );
			} else {
				messagesContentElem.find( '.bp-messages-content-wrapper' ).removeClass( 'scrolled--up' );
			}
		},
		...
		removeFeedback: function() {
			...
			if ( ! _.isUndefined( this.views.get( 'feedback' ) ) ) {
				...
				if ( _.isUndefined( feedback.get( 'view' ).model.attributes.type ) || 'notice' !== feedback.get( 'view' ).model.attributes.type ) {
					...
					$( '.bb-rl-messages-content' ).removeClass( 'has_info' );
					$( '.bb-rl-messages-nav-panel' ).removeClass( 'has_info' );
				}
			}
		},
		...
		displayComposeFeedback: function( message, type ) {
			...
			$( '.bb-rl-messages-content' ).addClass( 'has_info' );
		},
		...
		displaySearchFeedback: function( message, type ) {
			...
			feedback.inject( '.bb-rl-messages-search-feedback' );

			$( '.bb-rl-messages-nav-panel' ).addClass( 'has_info' );
		},
		...
		composeView: function() {
			...
			form.inject( '.bb-rl-messages-content' );

			$( '.bb-rl-messages-content' ).prepend( '<div class="compose-feedback"></div>' );

			// show compose message screen.
			$( '.bb-rl-messages-container' ).removeClass( 'bb-rl-view-message bp-compose-message' );
		...
		threadsView: function() {
			...
			thread_nav.inject( '#bb-rl-messages-thread-list-nav' );
			this.setupNav();
			...
			threads_list.inject( '.bb-rl-messages-threads-list' );
		...
		displayFilters: function( collection ) {
			...
			if ( collection.length ) {
				...
				filters_view.inject( '.bb-rl-messages-filters' );
				...
				$( '.bb-rl-messages-threads-list .bb-rl-message-lists > li .bb-rl-thread-subject' ).each( function () {
					var available_width = $( this ).width() - 10;
					var date_width = $( this ).find( '.bb-rl-thread-date' ).width();
					$( this ).find( '.bb-rl-thread-excerpt' ).css( { 'max-width': available_width - date_width } );
		...
		singleView: function( thread ) {
			...
			single_thread.inject( '.bb-rl-messages-content' );
		},
			...
			$.ajax(
				{
					...
					success: function ( response ) {
						if ( response.success && response.data && '' !== response.data.content ) {
							var membersListElem = $( '#message-members-list #members_list' );
		...
		messageBlockListPagination: function ( e ) {
			e.preventDefault();
			var viewMoreMembers = $( '#view_more_members' );
			if ( viewMoreMembers.length ) {
				viewMoreMembers.removeClass( 'view_more_members' );
			}
			var loadMoreRl = $( '#load_more_rl' );
			if ( loadMoreRl.length ) {
				loadMoreRl.removeClass( 'load_more_rl' );
			}
			var $this        = $( this );
			...
			var postData     = {
				...
				'thread_id': threadId,
				'exclude_current_user': true,
				'exclude_moderated_members': 'bp_load_more' === bpAction,
			...
			$.ajax( {
							...
							$.each( memberData, function ( index, item ) {
								if ( '' !== item ) {
									if ( 'bp_load_more' === bpAction ) {
										var userItemWrapElem  = $( '.user-item-wrp:last' ),
										    cloneUserItemWrap = userItemWrapElem.clone();
										...
										if ( 'block' === memberAction ) {
											if ( true === item.is_user_blocked ) {
												...
												cloneUserItemWrap.find( '.user-actions .block-member' ).attr( 'data-bp-nonce', bbRlModerationContentNonce );
												cloneUserItemWrap.find( '.user-actions .block-member' ).html( buttonTitle );
											}
											...
											if ( true === item.is_user_reported ) {
												...
												cloneUserItemWrap.find( '.user-actions .report-content' ).attr( 'data-bp-nonce', bbRlModerationContentNonce );
												cloneUserItemWrap.find( '.user-actions .report-content' ).html( buttonTitle );
											}
										}
										userItemWrapElem.after( cloneUserItemWrap );

									}
									if ( 'bp_view_more' === bpAction ) {
										var participantsNameElem  = $( '.participants-name:last' ),
										    oldSpanTagTextNode    = document.createTextNode( ', ' ),
										    cloneParticipantsName = participantsNameElem.clone();
										...
										if ( parseInt( index ) !== parseInt( Object.keys( memberData ).length ) || cloneParticipantsName ) {
											participantsNameElem.find( 'a' ).append( oldSpanTagTextNode );
										}
										participantsNameElem.after( cloneParticipantsName );
									}
								}
							} );
						}
				...
				complete: function () {
					var loadMoreRl = $( '#load_more_rl' );
					if ( loadMoreRl.length ) {
						loadMoreRl.removeClass( 'loading' );
						loadMoreRl.addClass( 'load_more_rl' );
					}
					if ( viewMoreMembers.length ) {
						viewMoreMembers.addClass( 'view_more_members' );
					}
				},
			} );
			return false;
		...
		messageBlockMember: function ( e ) {
			...
			var blockMember = $( '#mass-user-block-list a.block-member' );
			if ( blockMember.length ) {
		...
		messageReportMember: function( e ) {
			...
			var reportContent = $( '#mass-user-block-list a.report-content' );
			if ( reportContent.length ) {
				...
				if ( ! $( '#bb-report-content .form-item-category:visible label input[type="radio"]' ).length ) {
					var reportCategoryOther = $( '#report-category-other' );
					reportCategoryOther.attr( 'checked', true );
					reportCategoryOther.trigger( 'click' );
			...
			$.ajax(
				{
					...
					success: function ( response ) {
						if ( response.success && response.data && '' !== response.data.content ) {
							var moderatedUserList = $( '#mass-user-block-list #moderated_user_list' );
		...
		clearModeratedMessageList: function () {
			var moderatedUserList = $( '#moderated_user_list' );
			if ( moderatedUserList.length ) {
				moderatedUserList.html( '' ).removeClass( 'is_not_empty' );
			}
		},
		...
		threadAction: function ( event, obj ) {
			...
			if ( parseInt( thread_id ) === parseInt( $( event.currentTarget ).closest( '#bp-message-thread-header' ).find( '.message_action__list' ).data( 'bp-thread-id' ) ) || $( event.currentTarget ).closest( '.bb-rl-thread-item' ).hasClass( 'current' ) ) {
			...
			if (
				...
				target.parents( '.bb-rl-message-thread-options' ).addClass( 'loading' );
			}

			if (1 === $(event.currentTarget).closest('#bp-message-thread-header').length && ('delete' === action || 'delete_thread' === action)) {
				var messageActionAnchor = $( '.message_actions .message_action__anchor' );
				if ( messageActionAnchor.length ) {
					messageActionAnchor.trigger( 'click' );
				}
			}

			if ( 'delete' === action ) {
				if ( ! confirm( bbRlDeleteConfirmation ) ) {
					bp.Nouveau.Messages.removeFeedback();
					target.parents( '.message_actions' ).removeClass( 'loading' );
					target.parents( '.bb-rl-message-thread-options' ).removeClass( 'loading' );
					return;
				}
			}

			if ( 'delete_thread' === action ) {
				if ( ! confirm( bbRlDeleteThreadConfirmation ) ) {
					bp.Nouveau.Messages.removeFeedback();
					target.parents( '.message_actions' ).removeClass( 'loading' );
					target.parents( '.bb-rl-message-thread-options' ).removeClass( 'loading' );
					return;
				...
				function( response ) {
					...
					if ( 'delete_thread' === action && ( 'undefined' === typeof bb_pusher_vars || ( 'undefined' !== typeof bb_pusher_vars.is_live_messaging_enabled && 'off' === bb_pusher_vars.is_live_messaging_enabled ) ) ) {
						...
						if ( 'unread' === action && ! _.isUndefined( response.ids ) ) {
							$( '.bp-compose-message.bb-rl-messages-container, .bb-rl-view-message.bb-rl-messages-container' ).removeClass( 'bp-compose-message bb-rl-view-message' );
							$.each(
								response.ids,
								function( index, value ) {
									$( '#bb-rl-messages-threads-list .bb-rl-message-lists .bb-rl-thread-item.' + value ).addClass( 'unread' );
								}
							);
							...
							$.each(
								response.ids,
								function( index, value ) {
									$( '#bb-rl-messages-threads-list .bb-rl-message-lists .bb-rl-thread-item.' + value ).removeClass( 'unread' );
								}
							);
						}
		...
		backToThreadList: function( event ) {
			...
			$( '.bb-rl-message-header-loading' ).removeClass( 'bp-hide' );
		...
		showToasts: function() {
			...
			if ( show_detail_page ) {
				$( '.bb-rl-messages-container' ).addClass( 'bb-rl-view-message' );
		...
		openArchivedPage: function( event ) {
			...
			$( '.bb-rl-message-header-loading' ).removeClass( 'bp-hide' );
			$( '#subsubnav' ).addClass( 'bp-hide' );
			$( '.bb-rl-messages-content' ).addClass( 'bp-hide' ).html( '' );
		...
		handleTabChange: function( e ) {
			...
			$this.closest( '.bb-rl-messages-tabs-list' ).find( '.bb-rl-messages-tab-item' ).removeClass( 'active' );
	...
	bp.Collections.Threads = Backbone.Collection.extend(
		{
			...
			sync: function( method, model, options ) {
				...
				// Add generic nonce.
				options.data.nonce       = bbRlNonces.messages;
				...
				if ( 'read' === method ) {
					...
					if (
						...
						$( '.bb-rl-messages-search-feedback' ).html( loader );
					}
	...
	bp.Collections.Messages = Backbone.Collection.extend(
		{
			...
			parse: function( resp ) {
				...
				var documentSupport = $( '#whats-new-messages-toolbar .post-media-document-support' );
				if ( ! _.isUndefined( resp.user_can_upload_document ) && documentSupport.length ) {
				...
				var mediaSupport = $( '#whats-new-messages-toolbar .post-media-photo-support' );
				if ( ! _.isUndefined( resp.user_can_upload_media ) && mediaSupport.length ) {
				...
				var videoSupport = $('#whats-new-messages-toolbar .post-media-video-support');
				if ( ! _.isUndefined( resp.user_can_upload_video ) && videoSupport.length ) {
				...
				var gifSupport = $( '#whats-new-messages-toolbar .post-media-gif-support' );
				if ( ! _.isUndefined( resp.user_can_upload_gif ) && gifSupport.length ) {
				...
				var postMediaEmojiSupport = $( '#whats-new-formatting-toolbar .post-media-emoji-support' );
				if ( ! _.isUndefined( resp.user_can_upload_emoji ) && postMediaEmojiSupport.length ) {
					if ( resp.user_can_upload_emoji ) {
						postMediaEmojiSupport.show();
					} else {
						postMediaEmojiSupport.hide();
					}
				}
				...
				if (
					0 === bp.Nouveau.Messages.divider.length &&
					'' !== bp.Nouveau.Messages.last &&
				...
				if ( ! _.isUndefined( window.MediumEditor ) ) {
					...
					bp.Nouveau.Messages.mediumEditor = new window.MediumEditor(
						'#message_content',
						{
							...
							imageDragging: false,
							anchor: {
								placeholderText: bbRlAnchorPlaceholderText,
								linkValidation: true
							}
						}
					);
					...
					if ( ! $( 'body' ).hasClass( 'bb-is-mobile' ) ) {
						bp.Nouveau.Messages.mediumEditor.subscribe( 'editableKeypress', function ( event ) {
							...
							if ( event.keyCode === 13 && event.shiftKey ) {
								...
								if ( 'ul' === node.parentNode.tagName.toLowerCase() || 'ol' === node.parentNode.tagName.toLowerCase() ) {
					...
					$( document ).on( 'keyup', '.bb-rl-messages-content .medium-editor-toolbar-input', function ( event ) {
					...
					if ( ! _.isUndefined( bbRlMedia ) &&
						...
						(
							(
								! _.isUndefined( bbRlMedia.emoji.messages ) &&
								bbRlMedia.emoji.messages
							) ||
							(
								! _.isUndefined( bbRlMedia.emoji.groups ) &&
								bbRlMedia.emoji.groups
							)
						)
					) {
	...
	bp.Views.MessagesMedia = bp.Nouveau.Messages.View.extend(
		{
			...
			open_media_uploader: function() {
				...
				bp.Nouveau.Messages.dropzone_options.previewTemplate = document.getElementsByClassName( 'message-post-media-template' ).length ? document.getElementsByClassName( 'message-post-media-template' )[ 0 ].innerHTML : '';
				...
				bp.Nouveau.Messages.dropzone.on(
					...
					function(file, response) {
						if ( response.data.id ) {
							...
							self.model.set( 'media', self.media );
							if ( total_uploaded_file <= bbRlMedia.maxFiles ) {
								bp.Nouveau.Messages.removeFeedback();
							}
						} else {
	...
	bp.Views.MessagesDocument = bp.Nouveau.Messages.View.extend(
		{
			...
			open_document_uploader: function() {
				...
				// Check to avoid error if Node is missing.
				bp.Nouveau.Messages.dropzone_document_options.previewTemplate = document.getElementsByClassName( 'message-post-document-template' ).length ? document.getElementsByClassName( 'message-post-document-template' )[ 0 ].innerHTML : '';
				...
				bp.Nouveau.Messages.dropzone.on(
					...
					function(file, response) {
						if ( response.data.id ) {
							...
							self.model.set( 'document', self.document );
							if ( total_uploaded_file <= bbRlDocument.maxFiles ) {
								bp.Nouveau.Messages.removeFeedback();
							}
				...
				bp.Nouveau.Messages.dropzone.on(
					...
					function(file,response) {
						...
						if ( file.accepted ) {
							...
							// }
							errorText =  bbRlMedia.invalid_file_type + '</br>' + response;
						}
						bp.Nouveau.Messages.displaySendMessageFeedback( errorText, 'error' );
						this.removeFile( file );
						Backbone.trigger( 'triggerMediaChange' );
	...
	bp.Views.MessagesVideo = bp.Nouveau.Messages.View.extend(
		{
			...
			open_video_uploader: function() {
				...
				// Check to avoid error if Node is missing.
				bp.Nouveau.Messages.dropzone_video_options.previewTemplate = document.getElementsByClassName( 'message-post-video-template' ).length ? document.getElementsByClassName( 'message-post-video-template' )[ 0 ].innerHTML : '';
				...
				bp.Nouveau.Messages.dropzone.on(
					...
					function(file, response) {
						...
						if ( response.data.id ) {
							...
							self.model.set( 'video', self.video );
							if ( total_uploaded_file <= bbRlVideo.maxFiles ) {
								bp.Nouveau.Messages.removeFeedback();
							}
						} else {
							var message = response.data.feedback;
				...
				bp.Nouveau.Messages.dropzone.on(
					...
					function( file, done ) {
						bp.Nouveau.Messages.removeFeedback();
						if ( 0 === file.size ) {
							done( bbRlMedia.empty_video_type );
						} else {
							done();
						}
						Backbone.trigger( 'triggerMediaChange' );
				...
				bp.Nouveau.Messages.dropzone.on(
					...
					function(file,response) {
						...
						if ( file.accepted ) {
							...
							// $( 'body' ).append( '<div id="bp-video-create-album" style="display: block;" class="open-popup"><transition name="modal"><div class="modal-mask bb-white bbm-model-wrap"><div class="modal-wrapper"><div id="boss-video-create-album-popup" class="modal-container has-folderlocationUI"><header class="bb-model-header"><h4>' + bbRlMedia.invalid_media_type + '</h4><a class="bb-model-close-button closeModalErrorPopup" href="#"><span class="dashicons dashicons-no-alt"></span></a></header><div class="bb-field-wrap"><p>' + response + '</p></div></div></div></div></transition></div>' );
							errorText = bbRlMedia.invalid_media_type + '</br>' + response;
						}
						bp.Nouveau.Messages.displaySendMessageFeedback( errorText, 'error' );
						this.removeFile( file );
	...
	bp.Views.MessagesToolbar = bp.Nouveau.Messages.View.extend(
		{
			...
			disabledButton: function () {
				bp.Nouveau.Messages.displaySendMessageFeedback( bbRlErrors.media_fail, 'info noMediaError' );
				bp.Nouveau.Messages.checkContentScroll();
			},
	...
	bp.Views.MessagesAttachments = bp.Nouveau.Messages.View.extend(
		{
			...
			initialize: function() {
				...
				if ( ! _.isUndefined( window.Dropzone ) && ! _.isUndefined( bbRlMedia ) ) {
					...
					// Check the media.
					if ( ! this.model.get( 'is_group' ) && bbRlMedia.messages_media_active ) {
						this.messagesMedia = new bp.Views.MessagesMedia( {model: this.model} );
						this.views.add( this.messagesMedia );
					} else if ( this.model.get( 'is_group' ) && bbRlMedia.group_media ) {
						this.messagesMedia = new bp.Views.MessagesMedia( {model: this.model} );
						this.views.add( this.messagesMedia );
					}

					// Check the document.
					if ( ! this.model.get( 'is_group' ) && bbRlMedia.messages_document_active ) {
						this.messagesDocument = new bp.Views.MessagesDocument( {model: this.model} );
						this.views.add( this.messagesDocument );
					} else if ( this.model.get( 'is_group' ) && bbRlMedia.group_document ) {
						this.messagesDocument = new bp.Views.MessagesDocument( {model: this.model} );
						this.views.add( this.messagesDocument );
					}

					// Check the video.
					if ( ! this.model.get( 'is_group' ) && bbRlVideo.messages_video_active ) {
						this.messagesVideo = new bp.Views.MessagesVideo( {model: this.model} );
						this.views.add( this.messagesVideo );
					} else if ( this.model.get( 'is_group' ) && bbRlVideo.group_video ) {
						this.messagesVideo = new bp.Views.MessagesVideo( {model: this.model} );
						this.views.add( this.messagesVideo );
					}
				}
	...
	bp.Views.messageForm = bp.Nouveau.Messages.View.extend(
		{
			...
			closeComposeForm: function( event ) {
				...
				$( '.bb-rl-messages-container' ).removeClass( 'bp-compose-message' );
			},
						...
						ajax: {
							...
							data: function(params) {
								return $.extend(
									...
									params,
									{
										nonce: bbRlMessagesNonces.load_recipient,
	...
	bp.Views.userThreads = bp.Nouveau.Messages.View.extend(
		{
			tagName   : 'div',
			className : 'bb-rl-messages-user-threads',
			loadingFeedback : false,

			events: {
				'click .bb-rl-message-link' : 'changePreview',
				'scroll' : 'scrolled',
				'click .close-conversation' : 'doAction',
				'click .bb-rl-message-thread-options > .bb_more_options_action' : 'ToggleOptions',
			...
			initialize: function() {
				var Views = [
					new bp.Nouveau.Messages.View( { tagName: 'ul', id: 'bb-rl-message-threads', className: 'bb-rl-message-lists' } )
				];
				...
				if ( BP_Nouveau.messages.hasThreads ) {
					...
					this.threadsFetchError( {}, { feedback : bbRlErrors.no_messages, type: 'info' } );
				}
			...
			requestThreads: function( hideLoader ) {
				hideLoader = typeof hideLoader !== 'undefined' ? hideLoader : false; // jshint ignore:line
			...
			updateThreadsList: function ( response, hideLoader ) {
				hideLoader = typeof hideLoader !== 'undefined' ? hideLoader : true; // jshint ignore:line
				var updatedThread = '';
					...
					this.collection.models.forEach(
						function ( thread ) {
							...
							if ( thread_id === parseInt( response.thread_id ) ) {
								...
								if ( $( document.body ).find( '#bb-rl-message-threads li.' + thread_id ).hasClass( 'current' ) ) {
								...
								if ( '' === response.message.excerpt ) {
									...
									if ( ! _.isUndefined( response.message.media )  ) {
										response.message.excerpt = bbRlSingleMedia;
										if ( response.message.media.length > 1 ) {
											response.message.excerpt = bbRlMultipleMedia;
										}
									}
									if ( ! _.isUndefined( response.message.video ) ) {
										response.message.excerpt = bbRlSingleVideo;
										if ( response.message.video.length > 1 ) {
											response.message.excerpt = bbRlMultipleVideo;
										}
									}
									if ( ! _.isUndefined( response.message.document ) ) {
										response.message.excerpt = bbRlSingleDocument;
										if ( response.message.document.length > 1 ) {
											response.message.excerpt = bbRlMultipleDocument;
										}
									}
									if ( ! _.isUndefined( response.message.gif ) ) {
										response.message.excerpt = bbRlGifMedia;
									}
								}
			...
			threadsFetched: function() {
				...
				if ( this.collection.length ) {
					var $messagesContainer = $( '.bb-rl-messages-container' );
					$( '.bb-rl-messages-threads-list' ).removeClass( 'bp-no-messages' ).closest( '.bb-rl-messages-container' ).removeClass( 'bp-no-messages' );
					$messagesContainer.find( '.bb-rl-messages-nav-panel.loading' ).removeClass( 'loading' );
					$( '.bb-rl-message-header-loading' ).addClass( 'bp-hide' );
					$( '#subsubnav' ).removeClass( 'bp-hide' );
					$( '.bb-rl-messages-content' ).removeClass( 'bp-hide' );
					bp.Nouveau.Messages.displayFilters( this.collection );
					if (
						bp.Nouveau.Messages.stripTrailingSlash( window.location.href ) === bp.Nouveau.Messages.stripTrailingSlash( bbRlMessageUrl ) ||
						bp.Nouveau.Messages.stripTrailingSlash( window.location.href ) === bp.Nouveau.Messages.stripTrailingSlash( bbRlMessageArchivedUrl )
					) {
						if ( isMobile ) {
							$messagesContainer.removeClass( 'bb-rl-view-message' );
						}
			...
			threadsFetchError: function( collection, response ) {
				...
				if ( ! collection.length && ( 'undefined' === typeof collection.hideLoader || false === collection.hideLoader ) ) {
					$( '.bb-rl-messages-threads-list' ).addClass( 'bp-no-messages' ).closest( '.bb-rl-messages-container' ).addClass( 'bp-no-messages' );
					$( '.bb-rl-messages-container' ).find( '.bb-rl-messages-nav-panel.loading' ).removeClass( 'loading' );
					$( '.bp-messages.bp-user-messages-loading' ).remove();
					$( '.bb-rl-message-header-loading' ).addClass( 'bp-hide' );
					$( '#subsubnav' ).addClass( 'bp-hide' );
					...
					if ( 'archived' === bp.Nouveau.Messages.threadType ) {
						...
						this.views.add( new bp.Views.MessagesNoUnreadThreads() );
					} else {
			...
			scrolled: function( event ) {
				...
				if ( target.scrollTop() > 5 ) {
					target.closest( '.bb-rl-messages-nav-panel' ).addClass( 'threads-scrolled' );
				} else {
					target.closest( '.bb-rl-messages-nav-panel' ).removeClass( 'threads-scrolled' );
				}
			...
			cleanContent: function() {
				_.each(
					this.views._views['#bb-rl-message-threads'],
					function( view ) {
						view.remove();
					}
				);
			...
			addThread: function( thread ) {
				...
				this.views.add( '#bb-rl-message-threads', new bp.Views.userThread( { model: thread } ), { at: this.collection.indexOf( thread ) } );
			},
			...
			ToggleOptions: function( event ) {
				$( event.currentTarget ).closest( '.bb-rl-thread-item' ).toggleClass( 'optionsOpen' ).siblings().removeClass( 'optionsOpen' );
			},
	...
	bp.Views.userThread = bp.Nouveau.Messages.View.extend(
		{
			...
			initialize: function() {
				...
				if ( this.model.get( 'is_group' ) && 1 === this.model.get( 'is_group_thread' ) ) {
					this.el.className += ' bb-rl-group-thread';
				}
			...
			singleSelect: function( event ) {
				...
				if ( hasChecked ) {
					...
					bp.Nouveau.Messages.displayFeedback( bbRlHowtoBulk, 'info' );
				} else {
	...
	bp.Views.messageFilters = bp.Nouveau.Messages.View.extend(
		{
			...
			threadsFiltered: function () {
				bp.Nouveau.Messages.removeFeedback();
				var $messagesNoThreads = $( '.bb-messages-no-thread-found' );
				$messagesNoThreads.remove();
				$( '.bb-messages-search-no-thread-found' ).remove();
				$messagesNoThreads.remove();
				$( '.messages-search-loader' ).remove();
				$( '.bb-rl-messages-threads-list .bb-rl-message-lists > li .bb-rl-thread-subject' ).each(
					function () {
						var available_width = $( this ).width() - 10;
						var date_width      = $( this ).find( '.bb-rl-thread-date' ).width();
						$( this ).find( '.bb-rl-thread-excerpt' ).css( { 'max-width': available_width - date_width } );
			...
			threadsFilterError: function( collection, response ) {
				...
				no_result_view.render().$el.appendTo( '.bb-rl-messages-user-threads' );
	...
	bp.Views.userMessages = bp.Nouveau.Messages.View.extend(
		{
			...
			triggerPusherMessage: function ( messagePusherData ) {
				...
				if ( $.inArray( split_date, bp.Nouveau.Messages.divider ) === -1 ) {
					...
					split_message.content         = bbRlToday;
				...
				if (
					$( document.body ).find( '#bb-rl-messages-threads-list li.' + first_message.thread_id ).length &&
					'undefined' !== typeof first_message.display_date_list
				) {
					var list_date = $( document.body ).find( '#bb-rl-messages-threads-list li.' + first_message.thread_id + ' .bb-rl-thread-date' );
				...
				$messageThreadList.animate( { scrollTop : $messageThreadList.prop( 'scrollHeight' ) }, 0 );

				var $video = $( '#bp-message-thread-list li:last-child video' );
				if ( $video.length > 0 ) {
			...
			triggerPusherUpdateErrorMessage: function ( messagePusherData ) {
				...
				if ( model ) {
					...
					if ( content.search( 'message_send_error' ) === -1 ) {
						...
						// scroll to bottom
						var $messageThreadList = $( '#bp-message-thread-list' );
						$messageThreadList.animate( { scrollTop : $messageThreadList.prop( 'scrollHeight' ) }, 100 );
						setTimeout( function () {
			...
			triggerPusherUpdateMessage: function ( messagePusherData ) {
				...
				if ( model ) {
					...
					var $messageThreadList = $( '#bp-message-thread-list' );
					$messageThreadList.animate( { scrollTop: $messageThreadList.prop( 'scrollHeight' )}, 150 );
				}
			},
			...
			messagesFetched: function( collection, response ) {
				...
				if ( this.firstFetch ) {
					...
					$messageThreadList.animate( { scrollTop : this.firstLi.position().top - this.firstLi.outerHeight() }, 0 );
				}

				var $singleMessageWrap = $( '.bp-single-message-wrap' );
				if ( $singleMessageWrap.hasClass( 'group-messages-highlight' ) ) {
					$singleMessageWrap.parents( '#bp-message-thread-list' ).addClass( 'group-message-thread' );
				}

				var $loadMore = $( '#bp-message-load-more' );
				...
				$messageThreadList.on( 'scroll', this.messages_scrolled );
				...
				if ( ! this.rightPanel || this.rightPanel.model.get( 'id' ) !== this.options.thread.get( 'id' ) ) {
					...
					var $rightPanel = $( '#bb-rl-messages-right-panel' );
					if ( $rightPanel.length ) {
						$rightPanel.html( this.rightPanel.render().el );
						$rightPanel.show();
					}
				}
				...
				if ( $messageThreadList.height() > MessageThreadHeight ) {
					var button = $loadMore.find( 'button' );
					if ( ! button.hasClass( 'loading' ) ) {
						button.trigger( 'click' );
						button.addClass( 'initial-load' );
					}
				}
				if ( $loadMore.find( 'button' ).hasClass( 'initial-load' ) ) {
					setTimeout( function () {
						$messageThreadList.animate( { scrollTop: $messageThreadList.prop( 'scrollHeight' ) }, 100 );
						$loadMore.find( 'button' ).removeClass( 'initial-load' );
					}, 0 );
				}
			...
			sendReply: function( event ) {
				...
				var $sendReplyButton = $( '#send_reply_button' );
				if ( 'undefined' === typeof bb_pusher_vars || 'undefined' === typeof bb_pusher_vars.is_live_messaging_enabled || 'off' === bb_pusher_vars.is_live_messaging_enabled ) {
					$sendReplyButton.prop( 'disabled', true ).addClass( 'loading' );
				}
				$sendReplyButton.closest( '#bp-message-content' ).removeClass( 'focus-in--content' );
			...
			replyError: function( response ) {
				...
				var $sendReplyButton = $( '#send_reply_button' );
				if ( 'undefined' === typeof bb_pusher_vars || 'undefined' === typeof bb_pusher_vars.is_live_messaging_enabled || 'off' === bb_pusher_vars.is_live_messaging_enabled ) {
					$sendReplyButton.prop( 'disabled', false ).removeClass( 'loading' );
				}
				$sendReplyButton.closest( '#bp-message-content' ).removeClass( 'focus-in--content' );
			},
	...
	bp.Nouveau.Messages.Router = Backbone.Router.extend(
		{
			...
			composeMessage: function() {
				...
				if ( ! _.isUndefined( bbRlMedia ) ) {
					...
					if ( bbRlMedia.gif.messages === false ) {
					...
					if ( bbRlMedia.emoji.messages === false ) {
				...
				if ( ! _.isUndefined( bbRlArchivedThreads ) && 0 < bbRlArchivedThreads.length ) {
					bp.Nouveau.Messages.displayLinkInNoThreads( 'archived' );
				}
				...
				var isMobile = window.matchMedia( 'only screen and (max-width: 1080px)' ).matches;
				if ( isMobile ) {
					$( '.bb-rl-messages-container' ).addClass( 'bp-compose-message' );
				}
			},
				...
				$.each(
					...
					function() {
						...
						if ( _this.data( 'thread-id' ) === thread_id ) {
							_this.closest( '.bb-rl-thread-item' ).addClass( 'current' );
							if ( _this.closest( '.bb-rl-thread-item' ).hasClass( 'unread' ) ) {
								_this.closest( '.bb-rl-thread-item' ).removeClass( 'unread' );
							}
						} else {
							_this.closest( '.bb-rl-thread-item' ).removeClass( 'current' );
						}
					}
				);
	...
	bp.Views.MessagesNoUnreadThreads = bp.Nouveau.Messages.View.extend(
		{
			tagName: 'div',
			className: 'bb-messages-no-thread-found',
