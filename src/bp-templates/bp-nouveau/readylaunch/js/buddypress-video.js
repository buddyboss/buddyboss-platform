...
( function ( exports, $ ) {
	...
	bp.Nouveau.Video = {
		...
		deleteVideo : function ( event ) {
			var self = this, target = $( event.currentTarget ), dir_label, albumHandled = false;
			event.preventDefault();
							...
							if ( response.success ) {
								...
								if (
									...
									if ( $( '#buddypress .bb-item-count' ).length > 0 && 'yes' !== bbRlVideo.is_video_directory ) {
										...
										buddyPressSelector.find( '.groups-nav ul li#videos-groups-li a span.count' ).text( response.data.video_group_count );
									}
								}
								// inject video.
								if ( 'undefined' !== typeof bp.Nouveau.Media && 'function' === typeof bp.Nouveau.Media.handleAlbumDelete && bp.Nouveau.Media.handleAlbumDelete( response.data, video ) ) {
