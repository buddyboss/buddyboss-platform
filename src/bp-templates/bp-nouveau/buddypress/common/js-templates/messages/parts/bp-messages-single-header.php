	...
	<header class="single-message-thread-header">
		...
		<dl class="thread-participants">
			...
			if ( 1 < data.group_name.length && data.is_group_thread ) {
				...
				#>
				<dd>
					<span class="thread-date"><?php esc_html_e( 'Joined', 'buddyboss-platform' ); ?> {{data.group_joined_date}}</span>
				</dd>
				<#
				}
			} else { #>
				<dd>
					<span class="thread-date"><?php esc_html_e( 'Started', 'buddyboss-platform' ); ?> {{data.started_date}}</span>
				</dd>
			<# } #>
		</dl>
		<# } #>
		<div class="actions">
			...
			if ( bp_current_user_can( 'bp_moderate' ) ) {
				...
				?>
				<div class="message_actions">
					<a href="#" class="message_action__anchor" aria-label="<?php esc_attr_e( 'More actions', 'buddyboss-platform' ); ?>"> <i class="bb-icon-f bb-icon-ellipsis-h"></i> </a>
					<div class="message_action__list bb_more_dropdown" data-bp-thread-id="{{ data.id }}">
						<?php bp_get_template_part( 'common/more-options-view' ); ?>
						<ul>
							<li class="unread">
