		...
		<ul class="conflict-activity-ul-li-comment">
			...
			if ( $can_move || bp_loggedin_user_id() === bp_get_document_user_id() || bp_current_user_can( 'bp_moderate' ) ) {
				if ( ! in_array( $db_privacy, array( 'forums', 'message' ), true ) ) {
					if ( $is_comment_doc ) {
						?>
						<li class="move_file document-action-class move-disabled" data-balloon-pos="down" data-balloon="<?php esc_attr_e( 'Documents inherits activity privacy in comment. You are not allowed to move.', 'buddyboss' ); ?>">
