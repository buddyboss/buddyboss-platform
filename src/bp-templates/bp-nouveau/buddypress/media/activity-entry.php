...
echo ( $more_media && $media_template->current_media === 4 ) ? ' no_more_option ' : '';
?>
" data-id="<?php echo esc_attr( bp_get_media_id() ); ?>">
			...
			<div class="media-action_list">
				<ul class="conflict-activity-ul-li-comment">
					...
					<li class="delete_file">
						<a class="media-file-delete" data-item-activity-id="<?php echo esc_attr( $id ); ?>" data-parent-activity-id="<?php bp_media_parent_activity_id(); ?>" data-item-from="activity" data-item-id="<?php bp_media_id(); ?>" data-type="media" href="#"><?php _e( 'Delete', 'buddyboss' ); ?></a>
					</li>
				</ul>
			</div>
		<?php } ?>
	...
	>
		...
		<?php
		if ( $media_template->media_count > 5 && $media_template->current_media == 4 ) {
			?>
