...
<p class="bp-help-text"><?php /* translators: 1: moderator plural role label, 2: organizer plural role label. */ printf( __( 'Manage group members; promote to %1$s, co-%2$s, or demote or ban.', 'buddyboss-platform' ), strtolower( get_group_role_label( $bp_current_group_id, 'moderator_plural_label_name' ) ), strtolower( get_group_role_label( $bp_current_group_id, 'organizer_plural_label_name' ) ) ); ?></p>
...
<dl class="groups-manage-members-list">
						...
						<li class="member-entry clearfix">
							...
							<?php if ( count( bp_group_admin_ids( false, 'array' ) ) > 1 ) : ?>
								...
								<p class="action text-links-list">
									<a class="button confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( $bp_org_user_id ); ?>"><?php /* translators: %s: member singular role label. */ printf( __( 'Demote to regular %s', 'buddyboss-platform' ), strtolower( get_group_role_label( $bp_current_group_id, 'member_singular_label_name' ) ) ); ?></a>
								</p>
	...
	<?php if ( bp_group_has_moderators() ) : ?>
		...
		<dd class="moderator-listing">
			...
			<p><?php /* translators: 1: moderator singular role label, 2: organizer plural role label. */ printf( __( 'When a group member is promoted to be a %1$s of the group, the member gains the ability to edit and delete any forum discussion within the group and delete any activity feed items, excluding those posted by %2$s.', 'buddyboss-platform' ), strtolower( get_group_role_label( $bp_current_group_id, 'moderator_singular_label_name' ) ), strtolower( get_group_role_label( $bp_current_group_id, 'organizer_plural_label_name' ) ) ); ?></p>
					...
					while ( bp_members() ) :
						...
						<li class="members-entry clearfix">
							...
							<div class="members-manage-buttons action text-links-list">
								<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => $bp_mod_user_id ) ); ?>" class="button confirm mod-promote-to-admin"><?php /* translators: %s: organizer singular role label. */ printf( __( 'Promote to co-%s', 'buddyboss-platform' ), strtolower( get_group_role_label( $bp_current_group_id, 'organizer_singular_label_name' ) ) ); ?></a>
								<a class="button confirm mod-demote-to-member" href="<?php bp_group_member_demote_link( $bp_mod_user_id ); ?>"><?php /* translators: %s: member singular role label. */ printf( __( 'Demote to regular %s', 'buddyboss-platform' ), strtolower( get_group_role_label( $bp_current_group_id, 'member_singular_label_name' ) ) ); ?></a>
							</div>
	...
	<dd class="general-members-listing">
		...
		<p><?php /* translators: 1: member singular role label, 2: member plural role label. */ printf( __( 'When a member joins a group, he or she is assigned the %1$s role by default. %2$s are able to contribute to the group’s discussions, activity feeds, and view other group members.', 'buddyboss-platform' ), strtolower( get_group_role_label( $bp_current_group_id, 'member_singular_label_name' ) ), get_group_role_label( $bp_current_group_id, 'member_plural_label_name' ) ); ?></p>
