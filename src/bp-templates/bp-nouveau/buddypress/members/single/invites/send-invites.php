...
<form action="<?php echo esc_url( bp_displayed_user_domain() . bp_get_invites_slug() ); ?>" method="post" class="standard-form" id="send-invite-form">
	...
	<table class="invite-settings bp-tables-user" id="<?php echo esc_attr( 'member-invites-table' ); ?>">
		<thead>
		<tr>
			<th class="title"><?php esc_html_e( 'Recipient Name', 'buddyboss-platform' ); ?></th>
			<th class="title"><?php esc_html_e( 'Recipient Email', 'buddyboss-platform' ); ?></th>
			<?php
			if ( true === $send_invite_member_type_allow ) {
				?>
				<th class="title"><?php esc_html_e( 'Profile Type', 'buddyboss-platform' ); ?></th>
				<?php
			}
			?>
		...
		for ( $i = 0; $i < $raw; $i++ ) {
			...
			<tr>
				<td class="field-name">
					<input type="text" name="invitee[<?php echo esc_attr( $i ); ?>][]" id="invitee_<?php echo esc_attr( $i ); ?>_title" value="<?php echo esc_attr( '' ); ?>" class="invites-input" <?php bp_form_field_attributes( 'invitee' ); ?> aria-label="<?php esc_html_e( 'Recipient Name', 'buddyboss-platform' ); ?>"/>
				</td>
				<td class="field-email">
					<input type="email" name="email[<?php echo esc_attr( $i ); ?>][]" id="email_<?php echo esc_attr( $i ); ?>_email" value="<?php echo esc_attr( '' ); ?>" class="invites-input" <?php bp_form_field_attributes( 'email' ); ?> aria-label="<?php esc_html_e( 'Recipient Email', 'buddyboss-platform' ); ?>"/>
				</td>
				<?php
				if ( true === $send_invite_member_type_allow ) {
