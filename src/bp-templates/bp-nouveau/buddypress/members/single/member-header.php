		...
		<div id="item-header-cover-image" class="<?php echo esc_attr( bp_disable_cover_image_uploads() ? 'bb-disable-cover-img' : 'bb-enable-cover-img' ); ?>">
			...
			<div id="item-header-avatar" class="<?php echo esc_attr( $moderation_class ); ?>">
				...
				if ( $bp_is_my_profile && ! bp_disable_avatar_uploads() ) {
					?>
					<a href="<?php bp_members_component_link( 'profile', 'change-avatar' ); ?>" class="link-change-profile-image bp-tooltip" data-balloon-pos="down" data-balloon="<?php esc_attr_e( 'Change Profile Photo', 'buddyboss-platform' ); ?>" aria-label="<?php esc_attr_e( 'Change Profile Photo', 'buddyboss-platform' ); ?>">
