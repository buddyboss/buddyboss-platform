	...
	<div class="page" id="register-page">
		...
		<?php if ( 'registration-disabled' == bp_get_current_signup_step() ) : ?>
			...
			do_action( 'bp_before_registration_disabled' ); ?>
				...
				<p><?php _e( 'User registration is currently not allowed.', 'buddyboss' ); ?></p>
		...
		<?php if ( 'request-details' == bp_get_current_signup_step() ) : ?>
			...
			<p><?php _e( 'Registering for this site is easy. Just fill in the fields below, and we\'ll get a new account set up for you in no time.', 'buddyboss' ); ?></p>
			...
			<div class="register-section" id="basic-details-section">
				...
				<h2><?php _e( 'Account Details', 'buddyboss' ); ?></h2>

				<label for="signup_username"><?php _e( 'Username', 'buddyboss' ); ?> <?php _e( '(required)', 'buddyboss' ); ?></label>
				<?php
				...
				<label for="signup_email"><?php _e( 'Email Address', 'buddyboss' ); ?> <?php _e( '(required)', 'buddyboss' ); ?></label>
				<?php
				...
				<label for="signup_password"><?php _e( 'Choose a Password', 'buddyboss' ); ?> <?php _e( '(required)', 'buddyboss' ); ?></label>
				<?php
				...
				<label for="signup_password_confirm"><?php _e( 'Confirm Password', 'buddyboss' ); ?> <?php _e( '(required)', 'buddyboss' ); ?></label>
				<?php
			...
			<?php if ( bp_is_active( 'xprofile' ) ) : ?>
				...
				<div class="register-section" id="profile-details-section">
					...
					<h2><?php _e( 'Profile Details', 'buddyboss' ); ?></h2>
						...
						<div<?php bp_field_css_class( 'editfield' ); ?>>
							...
							if ( bp_current_user_can( 'bp_xprofile_change_field_visibility' ) ) : ?>
								<p class="field-visibility-settings-toggle" id="field-visibility-settings-toggle-<?php bp_the_profile_field_id() ?>"><span id="<?php bp_the_profile_field_input_name(); ?>-2">
									...
									<button type="button" class="visibility-toggle-link" aria-describedby="<?php bp_the_profile_field_input_name(); ?>-2" aria-expanded="false"><?php _ex( 'Change', 'Change profile field visibility level', 'buddyboss' ); ?></button>
								</p>
								...
								<div class="field-visibility-settings" id="field-visibility-settings-<?php bp_the_profile_field_id() ?>">
									...
									<button type="button" class="field-visibility-settings-close"><?php _e( 'Close', 'buddyboss' ) ?></button>
			...
			<?php if ( bp_get_blog_signup_allowed() ) : ?>
				...
				<div class="register-section" id="blog-details-section">
					...
					<h2><?php _e( 'Blog Details', 'buddyboss' ); ?></h2>

					<p><label for="signup_with_blog"><input type="checkbox" name="signup_with_blog" id="signup_with_blog" value="1"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes, I\'d like to create a new site', 'buddyboss' ); ?></label></p>

					<div id="blog-details"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?>class="show"<?php endif; ?>>

						<label for="signup_blog_url"><?php _e( 'Blog URL', 'buddyboss' ); ?> <?php _e( '(required)', 'buddyboss' ); ?></label>
						<?php
						...
						<label for="signup_blog_title"><?php _e( 'Site Title', 'buddyboss' ); ?> <?php _e( '(required)', 'buddyboss' ); ?></label>
						<?php
						...
						<fieldset class="register-site">
							...
							<label for="signup_blog_privacy_public"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_public" value="public"<?php if ( 'public' == bp_get_signup_blog_privacy_value() || !bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes', 'buddyboss' ); ?></label>
							<label for="signup_blog_privacy_private"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_private" value="private"<?php if ( 'private' == bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'No', 'buddyboss' ); ?></label>
						</fieldset>
			...
			<div class="submit">
				<input type="submit" name="signup_submit" id="signup_submit" value="<?php esc_attr_e( 'Complete Sign Up', 'buddyboss' ); ?>" />
			</div>
		...
		<?php if ( 'completed-confirmation' == bp_get_current_signup_step() ) : ?>
			...
			<div id="template-notices" role="alert" aria-atomic="true">
				<?php if ( bp_registration_needs_activation() ) : ?>
					<p><?php _e( 'You have successfully created your account! To begin using this site you will need to activate your account via the email we have just sent to your address.', 'buddyboss' ); ?></p>
				<?php else : ?>
					<p><?php _e( 'You have successfully created your account! Please log in using the username and password you have just created.', 'buddyboss' ); ?></p>
				<?php endif; ?>
			</div>
