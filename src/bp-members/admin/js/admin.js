...
( function( $ ) {
    ...
    if ( repeater_set_count > 0 ) {
        ...
        $( '#profile-edit-form .repeater_group_outer' ).each( function () {
            ...
            if ( '' === title ) {
                ...
                $set.find( '.repeater_set_title' ).html( BB_Member_Admin.empty_field );
            } else {
	...
	// Edit button
    $( '#profile-edit-form' ).on( 'click', '.repeater_group_outer .repeater_set_edit', function ( e ) {
        e.preventDefault();
	...
	// Delete button
    $( '#profile-edit-form' ).on( 'click', '.repeater_group_outer .repeater_set_delete', function ( e ) {
    ...
    $( '#profile-edit-form #btn_add_repeater_set' ).click( function ( e ) {
        ...
        $.ajax( {
            ...
            'data': {
                ...
                'user_id': $( '#user_id' ).val(),
                'origin': 'admin',
                'set_no': $( '#profile-edit-form input[name="repeater_set_sequence"]' ).val(),
                'existing_field_ids': $( '#profile-edit-form input[name="field_ids[]"]' ).val(),
            },
            'success': function ( response) {
                if ( response.success && response.data ) {
                    $( '.repeater_group_outer' ).last().after( response.data.html );
                    $( '#profile-edit-form [name="field_ids[]"]' ).val( response.data.field_ids.join( ',' ) );
                    $( '#profile-edit-form [name="repeater_set_sequence"]' ).val( response.data.set_no.join( ',' ) );
                    $( '.repeater_sets_sortable' ).sortable( 'destroy' ).sortable();
                    $( '.repeater_group_outer' ).last().addClass( 'active' );
                    $( '.repeater_group_outer .repeater_group_inner:last' ).show();
                ...
                // window.location.href = $( '#current_url' ).val() + '#bpxpro';
            }
        } );
    } );
    ...
    $( '#profile-edit-form' ).on( 'click', '.repeater_group_outer .visibility-toggle-link', function ( e ) {
        e.preventDefault();
    ...
    $( '#profile-edit-form' ).on( 'click', '.repeater_group_outer .field-visibility-settings-close', function ( e ) {
        e.preventDefault();
        ...
        settings_div.removeClass( 'field-visibility-settings-open' ).addClass( 'bp-hide' )
            .siblings( '.field-visibility-settings-toggle' )
            .find( '.current-visibility-level' ).text( vis_setting_text )
            .closest( '.field-visibility-settings-toggle' )
            .addClass( 'bp-show' )
            .removeClass( 'field-visibility-settings-hide bp-hide' );
    ...
    $( '#profile-edit-form input:not(:submit), #profile-edit-form textarea, #profile-edit-form select, #signup_form input:not(:submit), #signup_form textarea, #signup_form select' ).change( function () {
        ...
        window.onbeforeunload = function () {
            if ( shouldconfirm ) {
                return BB_Member_Admin.unsaved_changes;
            }
        };
    } );
