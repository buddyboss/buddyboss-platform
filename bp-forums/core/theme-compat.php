 ...
 * to replace the_content() with the appropriate Forums content.
 *
 * To do this, Forums does several direct manipulations of global variables
...
/**
 * Gets the name of the Forums compatable theme used, in the event the
 * currently active WordPress theme does not explicitly support Forums.
 * This can be filtered or set manually. Tricky theme authors can override the
 * default and include their own Forums compatibility layers for their themes.
 *
 * @since bbPress (r3506)
 * @uses apply_filters()
 * @return string
...
/**
 * Gets the version of the Forums compatable theme used, in the event the
 * currently active WordPress theme does not explicitly support Forums.
 * This can be filtered or set manually. Tricky theme authors can override the
 * default and include their own Forums compatibility layers for their themes.
 *
...
/**
 * Gets the Forums compatable theme used in the event the currently active
 * WordPress theme does not explicitly support Forums. This can be filtered,
 * or set manually. Tricky theme authors can override the default and include
 * their own Forums compatibility layers for their themes.
 *
 * @since bbPress (r3032)
 * @uses apply_filters()
 * @return string
 ...
 * if Forums was able to find a more appropriate template.
 *
 * @since bbPress (r3926)
 */
...
/**
 * Reset main query vars and filter 'the_content' to output a Forums
...
function bbp_template_include_theme_compat( $template = '' ) {
	...
	/**
	 * If BuddyBoss is activated at a network level, the action order is
	 * reversed, which causes the template integration to fail. If we're looking
	 * at a BuddyBoss page here, bail to prevent the extra processing.
	 *
	...
	/**
	 * Bail if the template already matches a Forums template. This includes
	 * archive-* and single-* WordPress post_type matches (allowing
	 * themes to use the expected format) as well as all Forums-specific
	...
	if ( bbp_is_template_included() ) {
		...
		/**
		 * If we are relying on Forums' built in theme compatibility to load
