...
class BB_CRM_Auto_Admin {
	...
	public function register_menus() {
		// Top-level Automations menu — works without BuddyBoss CRM.
		add_menu_page(
			__( 'Automations', 'buddyboss-automations' ),
			__( 'Automations', 'buddyboss-automations' ),
			'manage_options',
			'buddyboss-automations',
		...
		add_submenu_page(
			null,
			__( 'Edit Automation', 'buddyboss-automations' ),
			__( 'Edit Automation', 'buddyboss-automations' ),
			'manage_options',
			'buddyboss-automation-edit',
		...
		add_submenu_page(
			null,
			__( 'Automation Log', 'buddyboss-automations' ),
			__( 'Automation Log', 'buddyboss-automations' ),
			'manage_options',
			'buddyboss-automation-log',
	...
	public function render_menu_icon_css() {
		?>
		<style type="text/css">
			#adminmenu li.toplevel_page_buddyboss-automations .wp-menu-image:before {
				content: "\edc8";
	...
	public function enqueue_assets( $hook ) {
		if ( ! $hook || strpos( $hook, 'buddyboss-auto' ) === false ) return;
		...
		wp_localize_script( 'bb-crm-auto-admin', 'bbCrmAuto', array(
			...
			'strings'    => array(
				'confirm_delete' => __( 'Delete this automation? This cannot be undone.', 'buddyboss-automations' ),
				'add_action'     => __( '+ Add Step', 'buddyboss-automations' ),
				'add_condition'  => __( '+ Add Condition', 'buddyboss-automations' ),
				'remove'         => __( 'Remove', 'buddyboss-automations' ),
			),
		) );
	}
	...
	public function handle_save() {
		...
		wp_redirect( add_query_arg( array(
			'page'          => 'buddyboss-automation-edit',
	...
	public function handle_delete() {
		...
		wp_redirect( add_query_arg( array( 'page' => 'buddyboss-automations', 'deleted' => '1' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public function handle_toggle() {
		...
		wp_redirect( add_query_arg( array( 'page' => 'buddyboss-automations' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public function ajax_get_tags() {
		...
		global $wpdb;
		$rows = $wpdb->get_results( "SELECT id, name FROM {$wpdb->prefix}bb_tags ORDER BY name ASC" );
		$data = array();
		foreach ( $rows as $row ) {
	...
	public function ajax_get_lists() {
		...
		global $wpdb;
		$rows = $wpdb->get_results( "SELECT id, name FROM {$wpdb->prefix}bb_user_lists ORDER BY name ASC" );
		$data = array();
		foreach ( $rows as $row ) {
			$data[ $row->id ] = $row->name;
		}
	...
	public function ajax_get_automations() {
		...
		$data = array( 0 => __( 'All pending sequences', 'buddyboss-automations' ) );
		foreach ( $rows as $row ) {
			$data[ $row->id ] = $row->name;
		}
		wp_send_json_success( $data );
