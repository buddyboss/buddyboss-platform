...
<div class="wrap bb-crm-wrap">
	<h1>
		<?php if ( $auto ) : ?>
			<?php printf( esc_html__( 'Log: %s', 'buddyboss-automations' ), esc_html( $auto->name ) ); ?>
		<?php else : ?>
			<?php esc_html_e( 'Automation Log', 'buddyboss-automations' ); ?>
		<?php endif; ?>
	</h1>
	...
	<?php if ( empty( $logs ) ) : ?>
		<p><?php esc_html_e( 'No log entries yet. Logs will appear here once the automation fires.', 'buddyboss-automations' ); ?></p>
	<?php else : ?>
		<table class="wp-list-table widefat fixed striped">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Date', 'buddyboss-automations' ); ?></th>
					<?php if ( ! $auto ) : ?><th><?php esc_html_e( 'Automation', 'buddyboss-automations' ); ?></th><?php endif; ?>
					<th><?php esc_html_e( 'User', 'buddyboss-automations' ); ?></th>
					<th><?php esc_html_e( 'Trigger', 'buddyboss-automations' ); ?></th>
					<th><?php esc_html_e( 'Conditions', 'buddyboss-automations' ); ?></th>
					<th><?php esc_html_e( 'Status', 'buddyboss-automations' ); ?></th>
					<th><?php esc_html_e( 'Actions Result', 'buddyboss-automations' ); ?></th>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						...
						<td>
							<?php if ( $log->conditions_passed ) : ?>
								<span class="bb-crm-status-badge bb-crm-status-success">✓ <?php esc_html_e( 'Passed', 'buddyboss-automations' ); ?></span>
							<?php else : ?>
								<span class="bb-crm-status-badge bb-crm-status-warning">✗ <?php esc_html_e( 'Skipped', 'buddyboss-automations' ); ?></span>
							<?php endif; ?>
						</td>
						<td>
							<?php if ( $log->status === 'success' ) : ?>
								<span class="bb-crm-status-badge bb-crm-status-success"><?php esc_html_e( 'Success', 'buddyboss-automations' ); ?></span>
							<?php elseif ( $log->status === 'skipped' ) : ?>
								<span class="bb-crm-status-badge"><?php esc_html_e( 'Skipped', 'buddyboss-automations' ); ?></span>
							<?php else : ?>
								<span class="bb-crm-status-badge bb-crm-status-error"><?php esc_html_e( 'Failed', 'buddyboss-automations' ); ?></span>
							<?php endif; ?>
