<?php
/**
 * Plugin Name:       BuddyBoss Automations
 * Plugin URI:        https://github.com/tomjutla/buddyboss-crm
 * Description:       Automation workflows for BuddyBoss. Trigger actions based on member activity — no CRM required.
 * Version:           1.1.9
 ...
 * Text Domain:       buddyboss-automations
 *
 * @package BuddyBossAutomations
 */
...
final class BuddyBoss_CRM_Automations {
	...
	private function load() {
		...
		// Run install/upgrade when plugin version changes.
		if ( get_option( 'bb_crm_auto_db_version' ) !== BB_CRM_Auto_Install::DB_VERSION ) {
			BB_CRM_Auto_Install::install();
		}

		// Defer trigger loading so translations are available.
		...
		// Register prune cron.
		if ( ! wp_next_scheduled( 'bb_crm_auto_prune_log' ) ) {
			wp_schedule_event( time(), 'daily', 'bb_crm_auto_prune_log' );
		}
		add_action( 'bb_crm_auto_prune_log', array( $this, 'prune_log' ) );
	...
	private function setup_tables() {
		...
		$wpdb->bb_crm_automations      = $bb_prefix . 'bb_crm_automations';
		$wpdb->bb_crm_automation_log   = $bb_prefix . 'bb_crm_automation_log';
		$wpdb->bb_crm_automation_queue = $bb_prefix . 'bb_crm_automation_queue';
		$wpdb->bb_crm_email_opens      = $bb_prefix . 'bb_crm_email_opens';
	}

	public function load_triggers() {
		load_plugin_textdomain( 'buddyboss-automations', false, dirname( BB_CRM_AUTO_PLUGIN_BASENAME ) . '/languages' );
	...
	public function deactivate() {
		...
		wp_clear_scheduled_hook( 'bb_crm_auto_prune_log' );
	}

	public function prune_log() {
		$days = absint( get_option( 'bb_crm_auto_log_retention', 90 ) );
		if ( $days < 1 ) return;
		global $wpdb;
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
		$wpdb->query( $wpdb->prepare(
			"DELETE FROM {$wpdb->bb_crm_automation_log}
