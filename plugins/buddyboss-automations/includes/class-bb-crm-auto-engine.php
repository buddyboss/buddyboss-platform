...
class BB_CRM_Auto_Engine {
	...
	public function process_automation( $automation, $user_id, $trigger_data = array() ) {
		...
		$wpdb->query( $wpdb->prepare(
			"UPDATE {$wpdb->prefix}bb_crm_automations SET run_count = run_count + 1, last_run = NOW() WHERE id = %d",
			$automation->id
		) );
	...
	private function enqueue_actions( $automation_id, $user_id, $trigger_type, $trigger_data, $actions, $wait_config ) {
		...
		$wpdb->insert(
			$wpdb->prefix . 'bb_crm_automation_queue',
			array(
	...
	private function get_active_automations_for_trigger( $trigger_type ) {
		...
		return $wpdb->get_results( $wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}bb_crm_automations WHERE status = 'active' AND trigger_type = %s ORDER BY priority ASC",
			$trigger_type
		) );
	}
	...
	private function log( $automation_id, $user_id, $trigger_type, $trigger_data, $conditions_passed, $actions_result, $status, $error = '' ) {
		global $wpdb;
		$wpdb->insert(
			$wpdb->prefix . 'bb_crm_automation_log',
			array(
				'automation_id'     => $automation_id,
				'user_id'           => $user_id,
				'trigger_type'      => $trigger_type,
	...
	public static function get_automation( $id ) {
		...
		return $wpdb->get_row( $wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}bb_crm_automations WHERE id = %d",
			absint( $id )
		) );
	}
	...
	public static function get_automations( $args = array() ) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}bb_crm_automations WHERE {$where_sql} ORDER BY {$orderby} LIMIT %d OFFSET %d";
	...
	public static function count_automations( $status = '' ) {
		...
		if ( $status ) {
			return (int) $wpdb->get_var( $wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}bb_crm_automations WHERE status = %s",
				$status
			) );
		}
		return (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}bb_crm_automations" );
	}
	...
	public static function save_automation( $data ) {
		...
		if ( ! empty( $data['id'] ) ) {
			$wpdb->update( $wpdb->prefix . 'bb_crm_automations', $fields, array( 'id' => absint( $data['id'] ) ) );
			return absint( $data['id'] );
		}

		$wpdb->insert( $wpdb->prefix . 'bb_crm_automations', $fields );
	...
	public static function delete_automation( $id ) {
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . 'bb_crm_automation_log', array( 'automation_id' => absint( $id ) ) );
		return $wpdb->delete( $wpdb->prefix . 'bb_crm_automations', array( 'id' => absint( $id ) ) );
	}
	...
	public static function get_logs( $automation_id = 0, $per_page = 20, $page = 1 ) {
		...
		if ( $automation_id ) {
			return $wpdb->get_results( $wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}bb_crm_automation_log WHERE automation_id = %d ORDER BY created_at DESC LIMIT %d OFFSET %d",
				$automation_id, $per_page, $offset
			) );
		}

		return $wpdb->get_results( $wpdb->prepare(
			"SELECT l.*, a.name as automation_name FROM {$wpdb->prefix}bb_crm_automation_log l
			LEFT JOIN {$wpdb->prefix}bb_crm_automations a ON l.automation_id = a.id
